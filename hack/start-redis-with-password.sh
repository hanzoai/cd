...
KV_IMAGE_TAG=$(grep 'image: redis' manifests/base/redis/argocd-redis-deployment.yaml | cut -d':' -f3)
...
if [ "$CD_KV_LOCAL" = 'true' ]; then
    ...
    if [ -z "$KV_PASSWORD" ]; then
        ...
        docker run --rm --name argocd-redis -i -p "$KV_PORT:$KV_PORT" docker.io/library/redis:"$KV_IMAGE_TAG" --save '' --appendonly no --port "$KV_PORT"
    else
        echo "Starting Docker container with password."
        docker run --rm --name argocd-redis -i -p "$KV_PORT:$KV_PORT" -e KV_PASSWORD="$KV_PASSWORD" docker.io/library/redis:"$KV_IMAGE_TAG" redis-server --save '' --requirepass "$KV_PASSWORD" --appendonly no --port "$KV_PORT"
    fi
fi