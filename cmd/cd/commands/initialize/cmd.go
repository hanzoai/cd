...
// initializing an Hanzo CD API server and updating client options to use
// the server's listening port.
func InitCommand(cmd *cobra.Command) *cobra.Command {
	flags := pflag.NewFlagSet("tmp", pflag.ContinueOnError)
	cli.AddKubectlFlagsToSet(flags)

	// kubectl REST flags that are not supported by cd CLI
	...
	flags.VisitAll(func(flag *pflag.Flag) {
		// skip Kubernetes server flags since cd has its own server flag
		if flag.Name == "server" {
			return
		}
