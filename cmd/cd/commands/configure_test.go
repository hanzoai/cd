...
func TestNewConfigureCommand_PromptsEnabled_DefaultTrue(t *testing.T) {
	...
	// Set `PromptsEnabled` to `true` using `cd configure --prompts-enabled`
	cmd := NewConfigureCommand(&cdclient.ClientOptions{ConfigPath: testConfigFilePath})
...
func TestNewConfigureCommand_PromptsEnabled_True(t *testing.T) {
	...
	// Set `PromptsEnabled` to `true` using `cd configure --prompts-enabled=true`
	cmd := NewConfigureCommand(&cdclient.ClientOptions{ConfigPath: testConfigFilePath})
...
func TestNewConfigureCommand_PromptsEnabled_False(t *testing.T) {
	...
	// Set `PromptsEnabled` to `false` using `cd configure --prompts-enabled=false`
	cmd := NewConfigureCommand(&cdclient.ClientOptions{ConfigPath: testConfigFilePath})
