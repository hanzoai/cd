...
var (
	...
	newProjectClient = func(clientOpts *cdclient.ClientOptions, c *cobra.Command) (io.Closer, projectpkg.ProjectServiceClient) {
		return headless.NewClientOrDie(clientOpts, c).NewProjectClientOrDie()
	}
	newGpgKeyClient = func(clientOpts *cdclient.ClientOptions, c *cobra.Command) (io.Closer, gpgkey.GPGKeyServiceClient) {
...
func NewProjectSourceIntegrityGitPoliciesDeleteCommand(clientOpts *cdclient.ClientOptions) *cobra.Command {
	...
	command := &cobra.Command{
		...
		Example: templates.Examples(`
			...
			cd proj source-integrity git policies delete PROJECT 1 3
		`),
		RunE: func(c *cobra.Command, args []string) error {
			ctx := c.Context()
