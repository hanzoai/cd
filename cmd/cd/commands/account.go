...
import (
	...
	accountpkg "github.com/hanzoai/cd/pkg/apiclient/account"
	...
	sessionutil "github.com/hanzoai/cd/util/session"
...
`,
	...
	cd account update-password --account foobar
`,
		Run: func(c *cobra.Command, args []string) {
			ctx := c.Context()
...
func NewAccountGetUserInfoCommand(clientOpts *cdclient.ClientOptions) *cobra.Command {
	...
	command := &cobra.Command{
		...
		Example: templates.Examples(`
			# Get User information for the currently logged-in user (see 'cd login')
			cd account get-user-info

			# Get User information in yaml format
			cd account get-user-info -o yaml
		`),
		Run: func(c *cobra.Command, args []string) {
			ctx := c.Context()
...
cd account can-i update projects 'default'

# Can I create a cluster?
cd account can-i create clusters '*'
...
func getCurrentAccount(ctx context.Context, clientset cdclient.Client) session.GetUserInfoResponse {
...
cd account get --account <account-name>`,
		Run: func(c *cobra.Command, _ []string) {
			ctx := c.Context()
...
cd account generate-token

# Generate token for the account with the specified name
cd account generate-token --account <account-name>`,
		Run: func(c *cobra.Command, _ []string) {
			ctx := c.Context()
...
cd account delete-token ID

# Delete token of the account with the specified name
cd account delete-token --account <account-name> ID`,
		Run: func(c *cobra.Command, args []string) {
			ctx := c.Context()
...
cd account session-token

# Show detailed token information
cd account session-token -o json
		...
		Run: func(_ *cobra.Command, _ []string) {
			...
			if err != nil {
				if strings.Contains(err.Error(), "invalid_grant") && strings.Contains(err.Error(), "Invalid refresh_token") {
					log.Fatal("Refresh token is invalid or expired. Please run 'cd relogin' to re-authenticate")
				}
				log.Fatal(err)
			}
			...
			errors.CheckError(err)
			if localCfg == nil {
				log.Fatal("No configuration found. Please login first with 'cd login'")
			}

			configCtx, err := localCfg.ResolveContext(clientOpts.Context)
			errors.CheckError(err)
			if configCtx == nil {
				log.Fatal("No context found. Please login first with 'cd login'")
			}

			if configCtx.User.AuthToken == "" {
				log.Fatal("No authentication token found. Please login first with 'cd login'")
			}

			// Get token claims and validate
			claims, err := configCtx.User.Claims()
			if err != nil {
				log.Fatal("Invalid token format. Please run 'cd relogin'")
			}
			validator := jwt.NewValidator()
			if validator.Validate(claims) != nil {
				log.Fatal("Token is invalid or expired. Please run 'cd relogin'")
			}
