...
import (
	...
	eventspkg "github.com/hanzoai/cd/pkg/apiclient/events"
	gpgkeypkg "github.com/hanzoai/cd/pkg/apiclient/gpgkey"
	notificationpkg "github.com/hanzoai/cd/pkg/apiclient/notification"
...
func TestAppUrlDefault(t *testing.T) {
	t.Run("Plain text", func(t *testing.T) {
		result := appURLDefault(cdclient.NewClientOrDie(&cdclient.ClientOptions{
...
func TestTargetObjects(t *testing.T) {
	resources := []*v1alpha1.ResourceDiff{
		{
			TargetState: "{\"apiVersion\":\"v1\",\"kind\":\"Service\",\"metadata\":{\"name\":\"test-helm-guestbook\",\"namespace\":\"cd\"},\"spec\":{\"selector\":{\"app\":\"helm-guestbook\",\"release\":\"test\"},\"sessionAffinity\":\"None\",\"type\":\"ClusterIP\"},\"status\":{\"loadBalancer\":{}}}",
		},
		{
...
// `cd app wait --delete` does not short-circuit on the initial Get and
...
func (c *fakeAcdClient) ClientOptions() cdclient.ClientOptions {
	return cdclient.ClientOptions{}
}
