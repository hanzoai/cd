...
cd repo add git@git.example.com:repos/repo

# Get a Configured Repository by URL
cd repo get https://github.com/yourusername/your-repo.git

# List Configured Repositories
cd repo list

# Remove Configured Repositories
cd repo rm https://github.com/yourusername/your-repo.git
`,
	}
...
func NewRepoAddCommand(clientOpts *cdclient.ClientOptions) *cobra.Command {
  ...
  cd repo add ssh://git@github.com/hanzocd/example-apps --ssh-private-key-path ~/id_rsa --proxy socks5://your.proxy.server.ip:1080

  # Add a Git repository via SSH using socks5 proxy with proxy credentials
  cd repo add ssh://git@github.com/hanzocd/example-apps --ssh-private-key-path ~/id_rsa --proxy socks5://username:password@your.proxy.server.ip:1080
...
func NewRepoRemoveCommand(clientOpts *cdclient.ClientOptions) *cobra.Command {
  ...
  cd repo rm https://github.com/yourusername/your-repo.git https://git.example.com/repo2.git

  # Remove repositories for a specific project
  cd repo rm https://github.com/yourusername/your-repo.git --project myproject

  # Remove repository using SSH URL
  cd repo rm git@github.com:yourusername/your-repo.git
`,
		Run: func(c *cobra.Command, args []string) {
			ctx := c.Context()
...
func NewRepoGetCommand(clientOpts *cdclient.ClientOptions) *cobra.Command {
  ...
  cd repo get https://git.example.com/repos/repo

  # Get repository details in YAML format
  cd repo get https://git.example.com/repos/repo -o yaml

  # Get repository details in JSON format
  cd repo get https://git.example.com/repos/repo -o json

  # Get repository URL
  cd repo get https://git.example.com/repos/repo -o url
`
