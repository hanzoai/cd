...
func newProjectWindowsToggleCommand(clientOpts *cdclient.ClientOptions, use, short, long, example string, updateFn func(*v1alpha1.SyncWindow)) *cobra.Command {
...
func NewProjectWindowsDisableManualSyncCommand(clientOpts *cdclient.ClientOptions) *cobra.Command {
	return newProjectWindowsToggleCommand(
		...
		"Disable manual sync for a sync window. Requires ID which can be found by running \"cd proj windows list PROJECT\"",
		`
...
cd proj windows disable-manual-sync default 0`,
...
func NewProjectWindowsEnableManualSyncCommand(clientOpts *cdclient.ClientOptions) *cobra.Command {
	return newProjectWindowsToggleCommand(
		...
		"Enable manual sync for a sync window. Requires ID which can be found by running \"cd proj windows list PROJECT\"",
		`
...
cd proj windows enable-manual-sync default 2

#Enabling manual sync with a custom message
cd proj windows enable-manual-sync my-app-project --message "Manual sync initiated by admin`,
		func(window *v1alpha1.SyncWindow) {
...
func NewProjectWindowsDisableSyncOverrunCommand(clientOpts *cdclient.ClientOptions) *cobra.Command {
	return newProjectWindowsToggleCommand(
		...
		"Disable sync overrun for a sync window. Requires ID which can be found by running \"cd proj windows list PROJECT\"",
		`
...
cd proj windows disable-sync-overrun default 0`,
		func(window *v1alpha1.SyncWindow) {
...
func NewProjectWindowsEnableSyncOverrunCommand(clientOpts *cdclient.ClientOptions) *cobra.Command {
	return newProjectWindowsToggleCommand(
		...
		"Enable sync overrun for a sync window. When enabled on a deny window, syncs that started before the deny window will be allowed to continue. When enabled on an allow window, syncs that started during the allow window can continue after the window ends. Requires ID which can be found by running \"cd proj windows list PROJECT\"",
		`
#Enable sync overrun for a sync window
cd proj windows enable-sync-overrun PROJECT ID

#Enabling sync overrun for a window set on the default project with Id 2
cd proj windows enable-sync-overrun default 2`,
		func(window *v1alpha1.SyncWindow) {
...
func NewProjectWindowsDeleteCommand(clientOpts *cdclient.ClientOptions) *cobra.Command {
	command := &cobra.Command{
		...
		Short: "Delete a sync window from a project. Requires ID which can be found by running \"cd proj windows list PROJECT\"",
		Example: `
#Delete a sync window from a project (default) with ID 0
cd proj windows delete default 0

#Delete a sync window from a project (new-project) with ID 1
cd proj windows delete new-project 1`,
		Run: func(c *cobra.Command, args []string) {
			ctx := c.Context()
...
func NewProjectWindowsUpdateCommand(clientOpts *cdclient.ClientOptions) *cobra.Command {
	...
	command := &cobra.Command{
		...
		Long:  "Update a project sync window. Requires ID which can be found by running \"cd proj windows list PROJECT\"",
		Example: `# Change a sync window's schedule
cd proj windows update PROJECT ID \
    --schedule "0 20 * * *"
`,
		Run: func(c *cobra.Command, args []string) {
			ctx := c.Context()
...
cd proj windows list PROJECT -o yaml

#List project windows info for a project name (test-project)
cd proj windows list test-project`,
		Run: func(c *cobra.Command, args []string) {
			ctx := c.Context()
