...
import (
	...
	gpgkeymocks "github.com/hanzoai/cd/pkg/apiclient/gpgkey/mocks"
	projectmocks "github.com/hanzoai/cd/pkg/apiclient/project/mocks"
...
func mockProjectClient(t *testing.T) *projectmocks.ProjectServiceClient {
	...
	newProjectClient = func(_ *cdclient.ClientOptions, _ *cobra.Command) (io.Closer, projectpkg.ProjectServiceClient) {
...
func mockGpgKeysClient(t *testing.T) *gpgkeymocks.GPGKeyServiceClient {
	...
	newGpgKeyClient = func(_ *cdclient.ClientOptions, _ *cobra.Command) (io.Closer, gpgkey.GPGKeyServiceClient) {
		return io.NopCloser(nil), mockClient
	}
	return mockClient
}

// mockKeyring fakes that keys are added to a repo-server through `cd gpg add`, so those do not show up in warnings.
...
func TestProjectSourceIntegrityAddCommand(t *testing.T) {
	...
	t.Run("Add source integrity verification policy to empty", func(t *testing.T) {
		...
		cmd := NewProjectSourceIntegrityGitPoliciesAddCommand(&cdclient.ClientOptions{})
