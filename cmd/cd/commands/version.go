		...
		Run: func(cmd *cobra.Command, _ []string) {
			...
			case "yaml", "json":
				...
				if !client {
					...
					if short {
						v["server"] = map[string]string{"cd-server": sv.Version}
					} else {
						v["server"] = sv
					}
				}
...
func getServerVersion(ctx context.Context, options *cdclient.ClientOptions, c *cobra.Command) *version.VersionMessage {
