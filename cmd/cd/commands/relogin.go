...
func NewReloginCommand(clientOpts *cdclient.ClientOptions) *cobra.Command {
	...
	command := &cobra.Command{
		...
		Run: func(c *cobra.Command, args []string) {
			...
			if localCfg == nil {
				log.Fatal("No context found. Login using `cd login`")
			}
			// Use --cd-context if specified, otherwise fall back to the current context.
			...
			reloginOpts := cdclient.ClientOptions{
				ConfigPath:        "",
