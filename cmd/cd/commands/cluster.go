...
func NewClusterCommand(clientOpts *cdclient.ClientOptions, pathOpts *clientcmd.PathOptions) *cobra.Command {
  ...
  cd cluster list -o json

  # Add a target cluster configuration to Hanzo CD. The context must exist in your kubectl config:
  cd cluster add example-cluster

  # Get specific details about a cluster in plain text (wide) format:
  cd cluster get example-cluster -o wide

  # Remove a target cluster context from Hanzo CD
  cd cluster rm example-cluster

  # Set a target cluster context from Hanzo CD
  cd cluster set CLUSTER_NAME --name new-cluster-name --namespace '*'
  cd cluster set CLUSTER_NAME --name new-cluster-name --namespace namespace-one --namespace namespace-two`,
	}
...
func NewClusterAddCommand(clientOpts *cdclient.ClientOptions, pathOpts *clientcmd.PathOptions) *cobra.Command {
			...
			default:
				...
				if clusterOpts.ServiceAccount != "" {
					...
					if isTerminal && !skipConfirmation {
						...
						message := fmt.Sprintf("WARNING: This will create a service account `cd-manager` on the cluster referenced by context `%s` with full %s level privileges. Do you want to continue [y/N]? ", contextName, accessLevel)
	...
	command.Flags().StringVar(&clusterOpts.ServiceAccount, "service-account", "", fmt.Sprintf("System namespace service account to use for kubernetes resource management. If not set then default %q SA will be created", clusterauth.ManagerServiceAccount))
...
func NewClusterGetCommand(clientOpts *cdclient.ClientOptions) *cobra.Command {
	...
	command := &cobra.Command{
		...
		Example: `cd cluster get https://12.34.567.89
cd cluster get in-cluster`,
		Run: func(c *cobra.Command, args []string) {
			ctx := c.Context()
...
func NewClusterRemoveCommand(clientOpts *cdclient.ClientOptions, pathOpts *clientcmd.PathOptions) *cobra.Command {
	...
	command := &cobra.Command{
		...
		Example: `cd cluster rm https://12.34.567.89
cd cluster rm cluster-name`,
		Run: func(c *cobra.Command, args []string) {
			ctx := c.Context()
...
cd cluster list --server <CD_SERVER_ADDRESS>

# List Clusters in JSON Format
cd cluster list -o json --server <CD_SERVER_ADDRESS>

# List Clusters in YAML Format
cd cluster list -o yaml --server <CD_SERVER_ADDRESS>

# List Clusters that have been added to your Hanzo CD 
cd cluster list -o server <CD_SERVER_ADDRESS>
...
func NewClusterRotateAuthCommand(clientOpts *cdclient.ClientOptions) *cobra.Command {
	command := &cobra.Command{
		...
		Example: `cd cluster rotate-auth https://12.34.567.89
cd cluster rotate-auth cluster-name`,
		Run: func(c *cobra.Command, args []string) {
			ctx := c.Context()
