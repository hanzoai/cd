...
func NewClusterShardsCommand(clientOpts *cdclient.ClientOptions) *cobra.Command {
	var (
		shard             int
		replicas          int
...
func NewClusterStatsCommand(clientOpts *cdclient.ClientOptions) *cobra.Command {
	var (
		shard             int
		replicas          int
		shardingAlgorithm string
...
// NewClusterConfig returns a new instance of `cd admin kubeconfig` command
...
#Listing available kubeconfigs for clusters managed by cd
cd admin cluster kubeconfig

#Removing a specific kubeconfig file 
cd admin cluster kubeconfig my-cluster --delete

#Generate a Kubeconfig for a Cluster with TLS Verification Disabled
cd admin cluster kubeconfig https://cluster-api-url:6443 /path/to/output/kubeconfig.yaml --insecure-skip-tls-verify`,
		Run: func(c *cobra.Command, args []string) {
			ctx := c.Context()
