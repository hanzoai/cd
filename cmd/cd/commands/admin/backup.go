...
import (
	...
	secutil "github.com/hanzoai/cd/util/security"
)
...
func NewExportCommand() *cobra.Command {
	...
	command := cobra.Command{
		...
		Run: func(c *cobra.Command, _ []string) {
			...
			acdConfigMap, err := acdClients.configMaps.Get(ctx, common.ConfigMapName, metav1.GetOptions{})
			errors.CheckError(err)
			export(writer, *acdConfigMap, namespace)
			acdRBACConfigMap, err := acdClients.configMaps.Get(ctx, common.RBACConfigMapName, metav1.GetOptions{})
			errors.CheckError(err)
			export(writer, *acdRBACConfigMap, namespace)
			acdKnownHostsConfigMap, err := acdClients.configMaps.Get(ctx, common.KnownHostsConfigMapName, metav1.GetOptions{})
			errors.CheckError(err)
			export(writer, *acdKnownHostsConfigMap, namespace)
			acdTLSCertsConfigMap, err := acdClients.configMaps.Get(ctx, common.TLSCertsConfigMapName, metav1.GetOptions{})
			errors.CheckError(err)
	...
	command.Flags().StringSliceVarP(&applicationNamespaces, "application-namespaces", "", []string{}, fmt.Sprintf("Comma-separated list of namespace globs to export applications from, in addition to the control plane namespace (Hanzo CD namespace). "+
		...
		applicationNamespacesCmdParamsKey, common.CmdParamsConfigMapName))
	command.Flags().StringSliceVarP(&applicationsetNamespaces, "applicationset-namespaces", "", []string{}, fmt.Sprintf("Comma-separated list of namespace globs to export ApplicationSets from, in addition to the control plane namespace (Hanzo CD namespace). "+
		...
		applicationsetNamespacesCmdParamsKey, common.CmdParamsConfigMapName))
	return &command
}
...
func export(w io.Writer, un unstructured.Unstructured, cdNamespace string) {
	name := un.GetName()
