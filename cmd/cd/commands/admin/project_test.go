...
func TestUpdateProjects_FindMatchingProject(t *testing.T) {
	...
	err = updateProjects(ctx, clientset.AppsV1alpha1().AppProjects(namespace), "ba*", "*", "set", modification, false)
	require.NoError(t, err)

	fooProj, err := clientset.AppsV1alpha1().AppProjects(namespace).Get(ctx, "foo", metav1.GetOptions{})
	require.NoError(t, err)
	assert.Empty(t, fooProj.Spec.Roles[0].Policies)

	barProj, err := clientset.AppsV1alpha1().AppProjects(namespace).Get(ctx, "bar", metav1.GetOptions{})
	require.NoError(t, err)
...
func TestUpdateProjects_FindMatchingRole(t *testing.T) {
	...
	modification, err := getModification("set", "*", "*", "allow")
	require.NoError(t, err)
	err = updateProjects(ctx, clientset.AppsV1alpha1().AppProjects(namespace), "*", "fo*", "set", modification, false)
	require.NoError(t, err)

	proj, err := clientset.AppsV1alpha1().AppProjects(namespace).Get(ctx, "proj", metav1.GetOptions{})
	require.NoError(t, err)
