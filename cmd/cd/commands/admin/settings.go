...
func (opts *settingsOpts) createSettingsManager(ctx context.Context) (*settings.SettingsManager, error) {
	...
	switch {
	case opts.cdCMPath == "" && !opts.loadClusterSettings:
		return nil, stderrors.New("either --cd-cm-path must be provided or --load-cluster-settings must be set to true")
	case opts.cdCMPath == "":
	...
	default:
		data, err := os.ReadFile(opts.cdCMPath)
		if err != nil {
			return nil, err
		}
	...
	setSettingsMeta(cdCM)

	var cdSecret *corev1.Secret
	switch {
	case opts.cdSecretPath != "":
		data, err := os.ReadFile(opts.cdSecretPath)
		if err != nil {
			return nil, err
		}
		err = yaml.Unmarshal(data, &cdSecret)
		if err != nil {
			return nil, err
		}
		setSettingsMeta(cdSecret)
	...
	setSettingsMeta(cdSecret)
	clientset := fake.NewClientset(cdSecret, cdCM)
...
func NewSettingsCommand() *cobra.Command {
	...
	command.PersistentFlags().StringVar(&opts.cdCMPath, "cd-cm-path", "", "Path to local cd-cm.yaml file")
	command.PersistentFlags().StringVar(&opts.cdSecretPath, "cd-secret-path", "", "Path to local cd-secret.yaml file")
...
func NewValidateSettingsCommand(cmdCtx commandContext) *cobra.Command {
	...
	command := &cobra.Command{
		...
		Long:  "Validates settings specified in 'cd-cm' ConfigMap and 'cd-secret' Secret",
		Example: `
#Validates all settings in the specified YAML file
cd admin settings validate --cd-cm-path ./cd-cm.yaml

#Validates accounts and plugins settings in Kubernetes cluster of current kubeconfig context
cd admin settings validate --group accounts --group plugins --load-cluster-settings`,
...
func NewResourceIgnoreDifferencesCommand(cmdCtx commandContext) *cobra.Command {
	command := &cobra.Command{
		...
		Long:  "Renders ignored fields using the 'ignoreDifferences' setting specified in the 'resource.customizations' field of 'cd-cm' ConfigMap",
		Example: `
...
func NewResourceIgnoreResourceUpdatesCommand(cmdCtx commandContext) *cobra.Command {
	...
	command := &cobra.Command{
		...
		Long:  "Renders ignored fields using the 'ignoreResourceUpdates' setting specified in the 'resource.customizations' field of 'cd-cm' ConfigMap",
		Example: `
cd admin settings resource-overrides ignore-resource-updates ./deploy.yaml --cd-cm-path ./cd-cm.yaml`,
		Run: func(c *cobra.Command, args []string) {
			ctx := c.Context()
...
func NewResourceHealthCommand(cmdCtx commandContext) *cobra.Command {
	command := &cobra.Command{
		...
		Long:  "Assess resource health using the lua script configured in the 'resource.customizations' field of 'cd-cm' ConfigMap",
		Example: `
...
func NewResourceActionListCommand(cmdCtx commandContext) *cobra.Command {
	command := &cobra.Command{
		...
		Long:  "List actions available for given resource action using the lua scripts configured in the 'resource.customizations' field of 'cd-cm' ConfigMap and outputs updated fields",
		Example: `
cd admin settings resource-overrides action list /tmp/deploy.yaml --cd-cm-path ./cd-cm.yaml`,
		Run: func(c *cobra.Command, args []string) {
			ctx := c.Context()
...
func NewResourceActionRunCommand(cmdCtx commandContext) *cobra.Command {
	...
	command := &cobra.Command{
		...
		Long:    "Executes resource action using the lua script configured in the 'resource.customizations' field of 'cd-cm' ConfigMap and outputs updated fields",
		Example: `
cd admin settings resource-overrides action /tmp/deploy.yaml restart --cd-cm-path ./cd-cm.yaml`,
		Run: func(c *cobra.Command, args []string) {
			ctx := c.Context()
