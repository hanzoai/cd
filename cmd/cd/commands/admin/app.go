...
import (
	...
	"github.com/hanzoai/cd/controller/cache"
...
func NewGenAppSpecCommand() *cobra.Command {
	...
	cd admin app generate-spec kustomize-guestbook --repo https://github.com/hanzocd/example-apps.git --path kustomize-guestbook --dest-namespace default --dest-server https://kubernetes.default.svc --kustomize-image quay.io/argoprojlabs/cd-e2e-container:0.1
...
func NewReconcileCommand(clientOpts *cdclient.ClientOptions) *cobra.Command {
	var (
		...
		Run: func(c *cobra.Command, args []string) {
			...
			if refresh {
				...
				if repoServerAddress == "" {
					printLine("Repo server is not provided, trying to port-forward to cd-repo-server pod.")
				...
				repoServerClient := reposerverclient.NewRepoServerClientset(repoServerAddress, 60, tls.Configuration{DisableTLS: false, StrictValidation: false})
...
func reconcileApplications(
	...
	for _, app := range appsList.Items {
		...
		res, err := appStateManager.CompareAppState(ctx, &app, proj, revisions, sources, false, false, nil, false)
		if err != nil {
