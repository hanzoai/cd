...
import (
	...
	arogappsetv1 "github.com/hanzoai/deploy/pkg/apis/application/v1alpha1"
...
func NewApplicationSetGetCommand(clientOpts *cdclient.ClientOptions) *cobra.Command {
	...
	cd appset get APPSET_NAMESPACE/APPSETNAME

	# Get ApplicationSet in a specific namespace using --appset-namespace flag
	cd appset get --appset-namespace=APPSET_NAMESPACE APPSETNAME
		...
		Run: func(c *cobra.Command, args []string) {
			...
			appSetName, appSetNs := cd.ParseFromQualifiedName(args[0], appSetNamespace)
	...
	cd appset create --dry-run <filename or URL> -o json | jq -r '.status.resources[].name' 
		...
		Run: func(c *cobra.Command, args []string) {
			...
			for _, appset := range appsets {
				...
				conn, appIf := cdClient.NewApplicationSetClientOrDie()
				defer utilio.Close(conn)
				...
				if wait && !dryRun {
					err := waitForApplicationSetResourcesUpToDate(ctx, cdClient, created.QualifiedName())
					errors.CheckError(err)
...
func NewApplicationSetGenerateCommand(clientOpts *cdclient.ClientOptions) *cobra.Command {
	...
	cd appset generate <filename or URL> (<filename or URL>...)

	# Generate apps of ApplicationSet rendered templates in a specific namespace
	cd appset generate --appset-namespace=APPSET_NAMESPACE <filename or URL> (<filename or URL>...)
`),
		Run: func(c *cobra.Command, args []string) {
			ctx := c.Context()
...
func NewApplicationSetDeleteCommand(clientOpts *cdclient.ClientOptions) *cobra.Command {
	...
	cd appset delete APPSET_NAMESPACE/APPSETNAME

	# Delete ApplicationSet in a specific namespace using --appset-namespace flag
	cd appset delete --appset-namespace=APPSET_NAMESPACE APPSETNAME
		`),
		Run: func(c *cobra.Command, args []string) {
			ctx := c.Context()
			...
			for _, appSetQualifiedName := range args {
				appSetName, appSetNs := cd.ParseFromQualifiedName(appSetQualifiedName, appSetNamespace)
...
func waitForApplicationSetResourcesUpToDate(ctx context.Context, acdClient cdclient.Client, appSetName string) error {
