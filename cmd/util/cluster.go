...
// InClusterEndpoint returns true if Hanzo CD should reference the in-cluster
...
func AddClusterFlags(command *cobra.Command, opts *ClusterOptions) {
	command.Flags().BoolVar(&opts.InCluster, "in-cluster", false, "Indicates Hanzo CD resides inside this cluster and should connect using the internal k8s hostname (kubernetes.default.svc)")
	...
	command.Flags().StringVar(&opts.ExecProviderCommand, "exec-command", "", "Command to run to provide client credentials to the cluster. You may need to build a custom Hanzo CD image to ensure the command is available at runtime.")
