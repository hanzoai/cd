...
func NewCommand() *cobra.Command {
	command := &cobra.Command{
		...
		Short:             "cd-dex tools used by Hanzo CD",
		Long:              "cd-dex has internal utility tools used by Hanzo CD",
...
func NewRunDexCommand() *cobra.Command {
	...
	command := cobra.Command{
		...
		RunE: func(c *cobra.Command, _ []string) error {
			...
			errors.CheckError(err)
			config.UserAgent = fmt.Sprintf("cd-dex/%s (%s)", vers.Version, vers.Platform)
	...
	command.Flags().StringVar(&cmdutil.LogFormat, "logformat", env.StringFromEnv("CD_DEX_SERVER_LOGFORMAT", "json"), "Set the logging format. One of: json|text")
	command.Flags().StringVar(&cmdutil.LogLevel, "loglevel", env.StringFromEnv("CD_DEX_SERVER_LOGLEVEL", "info"), "Set the logging level. One of: debug|info|warn|error")
	command.Flags().BoolVar(&disableTLS, "disable-tls", env.ParseBoolFromEnv("CD_DEX_SERVER_DISABLE_TLS", false), "Disable TLS on the HTTP endpoint")
	return &command
}
