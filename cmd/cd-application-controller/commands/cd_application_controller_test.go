...
// cd-cmd-params-cm like the other controller parameters.
...
func TestNewCommand_MetricsFlagHelpUsesActualMetricNames(t *testing.T) {
	...
	assert.Contains(t, labelsFlag.Usage, "cd_app_labels")
	assert.NotContains(t, labelsFlag.Usage, "cd_application_labels")

	conditionsFlag := cmd.Flags().Lookup("metrics-application-conditions")
	require.NotNil(t, conditionsFlag)
	assert.Contains(t, conditionsFlag.Usage, "cd_app_condition")
	assert.NotContains(t, conditionsFlag.Usage, "cd_application_conditions")
}
