...
func TestNewCommand_RegistersMTLSFlags(t *testing.T) {
	...
	for _, flagName := range []string{
		...
		assert.NotNilf(t, cmd.Flags().Lookup(flagName),
			"expected flag %q to be registered on cd-server command", flagName)
	}
}
...
func TestNewCommand_MTLSEnvVarPrefix(t *testing.T) {
	t.Setenv("CD_SERVER_REPO_SERVER_CA_CERT_PATH", "/etc/certs/ca.crt")
	t.Setenv("CD_SERVER_REPO_SERVER_CLIENT_CERT_PATH", "/etc/certs/client.crt")
	t.Setenv("CD_SERVER_REPO_SERVER_CLIENT_CERT_KEY_PATH", "/etc/certs/client.key")
...
func TestNewCommand_MTLSEnvVarNotOverriddenByOtherComponents(t *testing.T) {
	t.Setenv("CD_APPLICATION_CONTROLLER_REPO_SERVER_CA_CERT_PATH", "/wrong/ca.crt")
	...
	assert.Empty(t, caCert,
		"APPLICATION_CONTROLLER env var must not affect cd-server's repo-server-ca-cert-path flag")
}
...
func TestNewCommand_CACertFlagRegistrationAndDefault(t *testing.T) {
	...
	require.NotNil(t, f, "flag \"repo-server-ca-cert-path\" must be registered on cd-server")
	...
	require.NotNil(t, clientCertFlag, "flag \"repo-server-client-cert-path\" must be registered on cd-server")
	...
	require.NotNil(t, clientCertKeyFlag, "flag \"repo-server-client-cert-key-path\" must be registered on cd-server")
