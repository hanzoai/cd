...
func (svc *argoCDService) GetCommitMetadata(ctx context.Context, repoURL string, commitSHA string, project string) (*shared.CommitMetadata, error) {
	cdDB := db.NewDB(svc.namespace, svc.settingsMgr, svc.clientset)
	repo, err := cdDB.GetRepository(ctx, repoURL, project)
	if err != nil {
		return nil, err
	}
...
func (svc *argoCDService) GetAppDetails(ctx context.Context, app *v1alpha1.Application) (*shared.AppDetail, error) {
	...
	cdDB := db.NewDB(svc.namespace, svc.settingsMgr, svc.clientset)
	repo, err := cdDB.GetRepository(ctx, appSource.RepoURL, app.Spec.Project)
	if err != nil {
		return nil, err
	}
	helmRepos, err := cdDB.ListHelmRepositories(ctx)
	if err != nil {
		return nil, err
	}
