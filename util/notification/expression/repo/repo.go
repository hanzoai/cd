...
import (
	...
	"github.com/hanzoai/deploy/util/notification/expression/shared"
...
func getAppDetails(un *unstructured.Unstructured, cdService service.Service) (*shared.AppDetail, error) {
	app, err := getApplication(un)
	if err != nil {
		return nil, err
	}
	appDetail, err := cdService.GetAppDetails(context.Background(), app)
	if err != nil {
		return nil, err
	}
...
func getCommitMetadata(commitSHA string, app *unstructured.Unstructured, cdService service.Service) (*shared.CommitMetadata, error) {
	...
	meta, err := cdService.GetCommitMetadata(context.Background(), repoURL, commitSHA, project)
	if err != nil {
		return nil, err
	}
	return meta, nil
...
func NewExprs(cdService service.Service, app *unstructured.Unstructured) map[string]any {
	return map[string]any{
		"RepoURLToHTTPS":    repoURLToHTTPS,
		"FullNameByRepoURL": FullNameByRepoURL,
		"QueryEscape":       url.QueryEscape,
		"GetCommitMetadata": func(commitSHA string) any {
			meta, err := getCommitMetadata(commitSHA, app, cdService)
			if err != nil {
				panic(err)
			}
		...
		"GetAppDetails": func() any {
			appDetails, err := getAppDetails(app, cdService)
			if err != nil {
				panic(err)
			}
