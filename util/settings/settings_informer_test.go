...
func testConfigMap() *corev1.ConfigMap {
	return &corev1.ConfigMap{Name: common.ConfigMapName, Namespace: "cd"}
}
...
func TestConfigMapEventHandler(t *testing.T) {
	...
	t.Run("a different configmap does not trigger a change", func(t *testing.T) {
		...
		other := &corev1.ConfigMap{Name: "cd-rbac-cm", Namespace: "cd"}
		h.OnAdd(other, false)
...
func TestRepositorySecretEventHandler(t *testing.T) {
	...
	t.Run("a non-repository secret does not trigger a change", func(t *testing.T) {
		...
		plain := &corev1.Secret{Name: "plain", Namespace: "cd"}
		h.OnAdd(plain, false)
