...
// Even in cases where the label name doesn't align well with Hanzo CD's other labels, we use the Kubernetes labels to
...
// except rest_client_ is replaced with cd_kubectl_.
//
// We use similar histogram bucket ranges, but reduce cardinality.
//
// We try to use similar labels, but we adjust to more closely match other Hanzo CD metrics.
//
...
var (
	clientCertRotationAgeGauge = prometheus.NewGaugeVec(prometheus.GaugeOpts{
		Name: "cd_kubectl_client_cert_rotation_age_seconds",
	...
	requestLatencyHistogram = prometheus.NewHistogramVec(prometheus.HistogramOpts{
		Name:    "cd_kubectl_request_duration_seconds",
	...
	resolverLatencyHistogram = prometheus.NewHistogramVec(
		prometheus.HistogramOpts{
			Name:    "cd_kubectl_dns_resolution_duration_seconds",
	...
	requestSizeHistogram = prometheus.NewHistogramVec(
		prometheus.HistogramOpts{
			Name: "cd_kubectl_request_size_bytes",
	...
	responseSizeHistogram = prometheus.NewHistogramVec(
		prometheus.HistogramOpts{
			Name: "cd_kubectl_response_size_bytes",
	...
	rateLimiterLatencyHistogram = prometheus.NewHistogramVec(
		prometheus.HistogramOpts{
			Name:    "cd_kubectl_rate_limiter_duration_seconds",
	...
	requestResultCounter = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Name: "cd_kubectl_requests_total",
	...
	execPluginCallsCounter = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Name: "cd_kubectl_exec_plugin_call_total",
	...
	requestRetryCounter = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Name: "cd_kubectl_request_retries_total",
	...
	transportCacheEntriesGauge = prometheus.NewGaugeVec(
		prometheus.GaugeOpts{
			Name: "cd_kubectl_transport_cache_entries",
	...
	transportCreateCallsCounter = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Name: "cd_kubectl_transport_create_calls_total",
...
//
// Since the metrics are global, this function only needs to be called once for a given Hanzo CD component.
//
