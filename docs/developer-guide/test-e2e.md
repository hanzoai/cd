...
The [/test/e2e/testdata](https://github.com/argoproj/argo-cd/tree/master/test/e2e/testdata) directory contains various Argo CD applications. Before test execution, the directory is copied into `/tmp/argo-e2e***` temp directory (configurable by `CD_E2E_DIR`) and used in tests as a
...
- `CD_E2E_APISERVER_PORT`: Listener port for `argocd-server` (default: `8080`)
- `CD_E2E_REPOSERVER_PORT`: Listener port for `argocd-reposerver` (default: `8081`)
...
If you have changed the port for `argocd-server`, be sure to also set `CD_SERVER` environment variable to point to that port, e.g. `export CD_SERVER=localhost:8888` before running `make test-e2e` so that the test will communicate to the correct server component.
