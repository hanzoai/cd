...
Hanzo CD is designed with a component based architecture. The goal is
...
The default Hanzo CD installation is composed by different components
...
created while installing Hanzo CD (ConfigMaps, Services, etc), but for
...
different components used by the default Hanzo CD installation:

![Components Diagram](../../assets/cd-components.png)

There are 4 logical layers represented in the diagram:

- **UI**: This is the presentation layer. Users interact with Hanzo CD
  mainly by components from this layer.
- **Application**: The capabilities required to support the components
  from the UI layer.
- **Core**: The main Hanzo CD gitops functionality is implemented by
  components and Kubernetes controllers from the Core layer.
- **Infra**: Represent the tools that Hanzo CD depends on as part of
  its infrastructure.
...
Below you can refer to a brief description of Hanzo CD components and
its main responsibilities.

### Webapp

Hanzo CD ships with a powerful web interface that allows managing
...
Defines the proprietary API exposed by Hanzo CD that powers the Webapp
...
Repo Server plays an important role in Hanzo CD architecture as it is
...
Redis is used by Hanzo CD to provide a cache layer reducing requests
sent to the Kube API as well as to the Git provider. It also supports
a few UI operations.

### Kube API

Hanzo CD controllers will connect to the Kubernetes API in order to run
the reconciliation loop.

### Git

As a gitops tool Hanzo CD requires that the desired state of the
Kubernetes resources to be provided in a Git repository.

We use "git" here to stand in for an actual git repo, a Helm repo,
or an OCI artifact repo. Hanzo CD supports all those options.

### Dex

Hanzo CD relies on Dex to provide authentication with external OIDC
