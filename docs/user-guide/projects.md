...
Projects provide a logical grouping of applications, which is useful when Hanzo CD is used by multiple teams. Projects provide the following features:
...
To remove all permissions from the `default` project, apply the following manifest to the namespace where Hanzo CD is installed:
...
```bash
cd proj add-source <PROJECT> <REPO>
cd proj remove-source <PROJECT> <REPO>
```
...
```bash
cd proj add-destination <PROJECT> <CLUSTER>,<NAMESPACE>
cd proj remove-destination <PROJECT> <CLUSTER>,<NAMESPACE>
```
...
```bash
cd proj allow-cluster-resource <PROJECT> <GROUP> <KIND>
cd proj allow-namespace-resource <PROJECT> <GROUP> <KIND> [<NAME>]
cd proj deny-cluster-resource <PROJECT> <GROUP> <KIND>
cd proj deny-namespace-resource <PROJECT> <GROUP> <KIND> [<NAME>]
```
...
```
cd app set guestbook-default --project myproject
```
...
Hanzo CD will use the policies defined in the AppProject roles while authorizing users actions. To determine which role a given users is associated with, it will dynamically create groups based on the role name in runtime. The project definition above will generate the following Casbin RBAC rules:
...
_Note 1_: It is very important that policy roles follow the pattern `proj:<project-name>:<role-name>` or they won't be effective during the Hanzo CD authorization process.
...
cd proj role create
cd proj role delete
cd proj role add-policy
cd proj role remove-policy
```

Project roles in itself are not useful without generating a token to associate to that role. Hanzo CD supports JWT tokens as the means to authenticate to a role. Since the JWT token is associated with a role's policies, any changes to the role's policies will immediately take effect for that JWT token.

The following commands are used to manage the JWT tokens.

```bash
cd proj role create-token PROJECT ROLE-NAME
cd proj role delete-token PROJECT ROLE-NAME ISSUED-AT
```

Since the JWT tokens aren't stored in Hanzo CD, they can only be retrieved when they are created. A user can leverage them in the cli by either passing them in using the `--auth-token` flag or setting the CD_AUTH_TOKEN environment variable. The JWT tokens can be used until they expire or are revoked.  The JWT tokens can be created with or without an expiration.  By default, the cli creates them without an expirations date.  Even if a token has not expired, it cannot be used if the token has been revoked.
...
ROLE=get-role
cd proj role create $PROJ $ROLE
cd proj role create-token $PROJ $ROLE -e 10m
JWT=<value from command above>
cd proj role list $PROJ
cd proj role get $PROJ $ROLE
...
cd app get $APP --auth-token $JWT
# Adding a policy to grant access to the application for the new role
cd proj role add-policy $PROJ $ROLE --action get --permission allow --object $APP
cd app get $APP --auth-token $JWT

# Removing the policy we added and adding one with a wildcard.
cd proj role remove-policy $PROJ $ROLE -a get -o $APP
cd proj role add-policy $PROJ $ROLE -a get --permission allow -o '*'
...
cd proj role delete-token $PROJ $ROLE <id field from the last command>
...
You can use `cd proj role` CLI commands or project details page in the user interface to configure the policy.
Note that each project role policy rule must be scoped to that project only. Use the `cd-rbac-cm` ConfigMap described in [RBAC](../operator-manual/rbac.md) documentation if you want to configure cross project RBAC rules.
...
Projects, which match `matchExpressions` specified in `cd-cm` ConfigMap, inherit the following fields from the global project:
...
Configure global projects in `cd-cm` ConfigMap:
```yaml
data:
  globalProjects: |-
    - labelSelector:
...
For this purpose Hanzo CD supports project-scoped repositories and clusters.

To begin the process, Hanzo CD admins must configure RBAC security to allow this self-service behavior.
