...
Hanzo CD supports several different ways in which Kubernetes manifests can be defined:
...
## Development
Hanzo CD also supports uploading local manifests directly. Since this is an anti-pattern of the
...
```bash
$ cd app sync APPNAME --local /path/to/dir/
```
