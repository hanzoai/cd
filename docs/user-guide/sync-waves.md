...
Hanzo CD also offers an alternative method of changing the sync order of resources. These are sync waves. They are defined by the apps.hanzo.ai/sync-wave annotation. The value is an integer that defines the ordering (Hanzo CD will start deploying from the lowest number and finish with the highest number).
