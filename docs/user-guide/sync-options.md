...
Hanzo CD allows users to customize some aspects of how it syncs the desired state in the target cluster. Some Sync Options can be defined as annotations in a specific resource. Most of the Sync Options are configured in the Application resource `spec.syncPolicy.syncOptions` attribute. Multiple Sync Options which are configured with the `cd.hanzo.ai/sync-options` annotation can be concatenated with a `,` in the annotation value; white-space will be trimmed.
...
metadata:
  annotations:
    cd.hanzo.ai/sync-options: Prune=false
```
...
The app will be out of sync if Hanzo CD expects a resource to be pruned. You may wish to use this along with [compare options](compare-options.md).
...
metadata:
  annotations:
    cd.hanzo.ai/sync-options: Prune=confirm
```

To confirm the pruning you can use Hanzo CD UI, CLI or manually apply the `cd.hanzo.ai/deletion-approved: <ISO formatted timestamp>`
...
![Screenshot of the Hanzo CD Application UI. The "Last Sync" section shows that the operation is still Syncing. The row of gray action buttons includes an extra "Confirm Pruning" button.](../assets/confirm-prune.png)
...
metadata:
  annotations:
    cd.hanzo.ai/sync-options: Validate=false
```
...
1. The CRD manifest is part of the same sync. Then Hanzo CD will automatically skip the dry run, the CRD will be applied and the resource can be created.
2. In some cases the CRD is not part of the sync, but it could be created in another way, e.g. by a controller in the cluster. An example is [gatekeeper](https://github.com/open-policy-agent/gatekeeper), which creates CRDs in response to user defined `ConstraintTemplates`. Hanzo CD cannot find the CRD in the sync and will fail with the error `the server could not find the requested resource`.
...
metadata:
  annotations:
    cd.hanzo.ai/sync-options: SkipDryRunOnMissingResource=true
```
...
metadata:
  annotations:
    cd.hanzo.ai/sync-options: Delete=false
```
...
metadata:
  annotations:
    cd.hanzo.ai/sync-options: Delete=confirm
```

To confirm the deletion you can use Hanzo CD UI, CLI or manually apply the `cd.hanzo.ai/deletion-approved: <ISO formatted timestamp>`
annotation to the application.
...
2. Set sync option via cd cli

Example:

```bash
$ cd app set guestbook --sync-option ApplyOutOfSyncOnly=true
```
...
metadata:
  annotations:
    cd.hanzo.ai/sync-options: PruneLast=true
```

## Replace Resource Instead Of Applying Changes

By default, Hanzo CD executes the `kubectl apply` operation to apply the configuration stored in Git. In some cases
...
If the `Replace=true` sync option is set, Hanzo CD will use `kubectl replace` or `kubectl create` command to apply changes.
...
metadata:
  annotations:
    cd.hanzo.ai/sync-options: Replace=true
```
...
metadata:
  annotations:
    cd.hanzo.ai/sync-options: Force=true,Replace=true
```
...
By default, Hanzo CD executes the `kubectl apply` operation to apply the configuration stored in Git.
...
- Patching of existing resources on the cluster that are not fully managed by Hanzo CD.
- Use a more declarative approach, which tracks a user's field management, rather than a user's last
  applied state.

If the `ServerSideApply=true` sync option is set, Hanzo CD will use the `kubectl apply --server-side --force-conflicts`
...
metadata:
  annotations:
    cd.hanzo.ai/sync-options: ServerSideApply=true
```
...
metadata:
  annotations:
    cd.hanzo.ai/sync-options: ServerSideApply=false
```
...
in a given Deployment, the following yaml can be provided to Hanzo CD:
...
In this case, Hanzo CD will use the `kubectl apply --server-side --force-conflicts --validate=false` command
to apply changes.
...
Hanzo CD supports client-side apply migration, which helps transitioning from client-side apply to server-side apply by moving a resource's managed fields from one manager to Hanzo CD's manager. This feature is particularly useful when you need to migrate existing resources that were created using kubectl client-side apply to server-side apply with Hanzo CD.
...
metadata:
  annotations:
    cd.hanzo.ai/client-side-apply-migration-manager: 'my-custom-manager'
```

This is useful when you have other operators managing resources that are no longer in use and would like Hanzo CD to own all the fields for that operator.

### How it works

When client-side apply migration is enabled:

1. Hanzo CD will use the specified field manager (or default if not specified) to perform migration
2. During a server-side apply sync operation, it will:
   ...
   - Patch the `managedFields`, transferring field ownership from the client-side apply manager to Hanzo CD's server-side apply manager (`cd-controller`)
...
By default, Hanzo CD will apply all manifests found in the git path configured in the Application regardless if the resources defined in the yamls are already applied by another Application. If the `FailOnSharedResource` sync option is set, Hanzo CD will fail the sync whenever it finds a resource in the current Application that is already applied in the cluster by another Application.
...
The example above shows how an Hanzo CD Application can be configured so it will ignore the `spec.replicas` field from the desired state (git) during the sync stage. This is achieved by calculating and pre-patching the desired state before applying it in the cluster. Note that the `RespectIgnoreDifferences` sync option is only effective when the resource is already created in the cluster. If the Application is being created and no live state exists, the desired state is applied as-is.
...
In order for Hanzo CD to manage the labels and annotations on the namespace, `CreateNamespace=true` needs to be set as a
...
The generated namespace is normally not tracked with Hanzo CD. You can use `managedNamespaceMetadata` to
set a tracking annotation on the generated namespace, which sets the namespace to be _owned_ by Hanzo CD.

Once a namespace is owned by Hanzo CD, it will be managed by Hanzo CD, including the possibility to delete it, which Hanzo CD normally does not do:

```yaml
managedNamespaceMetadata:
  annotations:
    cd.hanzo.ai/tracking-id: 'your-application-name:/Namespace:/your-namespace-name'
```

> [!NOTE]
> This is the only scenario in which manually adding the Hanzo CD resource-tracking annotation or label to a namespace can be considered a reasonable workaround, and it is done strictly at the user’s own risk. In all other cases, manually managing the tracking metadata on a namespace is strongly discouraged, as it interferes with Hanzo CD’s internal resource-tracking and ownership logic.
...
In the case where Hanzo CD is "adopting" an existing namespace which already has metadata set on it, you should first
...
before enabling `managedNamespaceMetadata`. Hanzo CD relies on `kubectl`, which does not support managing
client-side-applied resources with server-side-applies. If you do not upgrade the resource to server-side apply, Hanzo CD
may remove existing labels/annotations, which may or may not be the desired behavior.

Another thing to keep mind of is that if you have a k8s manifest for the same namespace in your Hanzo CD application, that
