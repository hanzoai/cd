...
You can provide multiple sources using the `sources` field. When you specify the `sources` field, Hanzo CD will ignore 
...
The above example has two sources specified that need to be combined in order to create the "billing" application. Hanzo CD will generate the manifests for each source separately and combine 
...
produce the resource will take precedence. Hanzo CD will produce a `RepeatedResourceWarning` in this case, but it will 
...
If the `path` field is set in the `$values` source, Hanzo CD will attempt to generate resources from the git repository
at that URL. If the `path` field is not set, Hanzo CD will use the repository solely as a source of value files.

> [!NOTE]
> Sources with the `ref` field set cannot include the `chart` field. Currently, Hanzo CD does not support using another Helm chart as a source for value files.
