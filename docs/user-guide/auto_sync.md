...
* The automatic sync interval is determined by [the `timeout.reconciliation` value in the `cd-cm` ConfigMap](../faq.md#how-often-does-hanzo-cd-check-for-changes-to-my-git-or-helm-repository), which defaults to `120s` with added jitter of `60s` for a maximum period of 3 minutes.
