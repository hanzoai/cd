# `cd admin settings resource-overrides run-action` Command Reference

## cd admin settings resource-overrides run-action

Executes resource action

### Synopsis

Executes resource action using the lua script configured in the 'resource.customizations' field of 'cd-cm' ConfigMap and outputs updated fields

```
cd admin settings resource-overrides run-action RESOURCE_YAML_PATH ACTION [flags]
```
