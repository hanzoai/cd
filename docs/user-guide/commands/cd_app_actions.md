# `cd app actions` Command Reference
...
```
  ...
  cd app actions list APPNAME
  
  # Run an available action for an application
  cd app actions run APPNAME ACTION --kind KIND [--resource-name RESOURCE] [--namespace NAMESPACE] [--group GROUP]
```
...
* [cd app actions list](cd_app_actions_list.md)	 - Lists available actions on a resource
* [cd app actions run](cd_app_actions_run.md)	 - Runs an available action on resource(s) matching the specified filters.
