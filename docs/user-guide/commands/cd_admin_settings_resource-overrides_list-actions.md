# `cd admin settings resource-overrides list-actions` Command Reference

## cd admin settings resource-overrides list-actions

List available resource actions

### Synopsis

List actions available for given resource action using the lua scripts configured in the 'resource.customizations' field of 'cd-cm' ConfigMap and outputs updated fields

```
cd admin settings resource-overrides list-actions RESOURCE_YAML_PATH [flags]
```
