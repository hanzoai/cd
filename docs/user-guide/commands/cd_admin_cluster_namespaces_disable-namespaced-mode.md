# `cd admin cluster namespaces disable-namespaced-mode` Command Reference

## cd admin cluster namespaces disable-namespaced-mode

Disable namespaced mode for clusters which name matches to the specified pattern.

```
cd admin cluster namespaces disable-namespaced-mode PATTERN [flags]
```
...
* [cd admin cluster namespaces](cd_admin_cluster_namespaces.md)	 - Print information namespaces which Hanzo CD manages in each cluster.
