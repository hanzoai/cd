...
```
  # Add a TLS certificate for cd.example.com to Hanzo CD cert store from a file
  cd cert add-tls --from ~/mycert.pem cd.example.com

  # Add a TLS certificate for cd.example.com to Hanzo CD via stdin
  cat ~/mycert.pem | cd cert add-tls cd.example.com

  # Add SSH known host entries for cd.example.com to Hanzo CD by scanning host
  ssh-keyscan cd.example.com | cd cert add-ssh --batch

  # List all known TLS certificates
  cd cert list --cert-type https

  # Remove all TLS certificates for cd.example.com
  cd cert rm --cert-type https cd.example.com

  # Remove all certificates and SSH known host entries for cd.example.com
  cd cert rm cd.example.com
...
* [cd cert rm](cd_cert_rm.md)	 - Remove certificate of TYPE for REPOSERVER
