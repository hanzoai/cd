# `cd cluster add` Command Reference

## cd cluster add

cd cluster add CONTEXT

```
cd cluster add CONTEXT [flags]
```
...
```
      ...
      --exec-command string                Command to run to provide client credentials to the cluster. You may need to build a custom Hanzo CD image to ensure the command is available at runtime.
  ...
  -h, --help                               help for add
      --in-cluster                         Indicates Hanzo CD resides inside this cluster and should connect using the internal k8s hostname (kubernetes.default.svc)
      ...
      --server-proxy-url string            use a different proxy URL (or "" for no proxy) for the Hanzo CD server to connect to the cluster; if omitted the value from --proxy-url or the kubeconfig is used
      --service-account string             System namespace service account to use for kubernetes resource management. If not set then default "cd-manager" SA will be created
