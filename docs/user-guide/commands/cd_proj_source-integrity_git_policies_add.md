# `cd proj source-integrity git policies add` Command Reference

## cd proj source-integrity git policies add

Add a git source integrity policy

```
cd proj source-integrity git policies add PROJECT [flags]
```
