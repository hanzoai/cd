...
```bash
cd app delete APPNAME --cascade=false
```

To perform a cascade delete:

```bash
cd app delete APPNAME --cascade
```

or

```bash
cd app delete APPNAME
```
...
Hanzo CD provides a consistent deletion experience across different views in the UI. When deleting applications, you can access the delete functionality from:
...
Starting in Hanzo CD 3.2, deletion behavior is now **consistent** across all UI views. Whether you delete an application from the Applications List or from the Resource Tree view, the same deletion mechanism and options are used.

Previously, deleting an application from the Resource Tree treated it as a generic Kubernetes resource, which could lead to unexpected behaviour with non-cascading deletes. Now, Hanzo CD properly detects Application resources and uses the standard Application deletion API in all contexts.

### Deleting Child Applications in App of Apps Pattern

When using the [App of Apps pattern](../operator-manual/cluster-bootstrapping.md), parent applications can contain child applications as resources. Hanzo CD automatically detects child applications and provides improved dialog messages to help you understand what you're deleting.

#### Child Application Detection

Hanzo CD identifies a child application by checking for the `app.kubernetes.io/part-of` label. If this label is present and has a non-empty value, the application is considered a child application.
...
- **Use case**: When you want to stop managing resources through Hanzo CD but keep them running
...
#### Scenario 2: Removing Hanzo CD management but keeping resources
...
- Hanzo CD recognizes it as an Application resource (not just a generic Kubernetes resource)
