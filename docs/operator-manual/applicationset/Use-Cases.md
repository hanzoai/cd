...
With the concept of generators, the ApplicationSet controller provides a powerful set of tools to automate the templating and modification of Hanzo CD Applications. Generators produce template parameter data from a variety of sources, including Hanzo CD clusters and Git repositories, supporting and enabling new use cases.
...
The infrastructure team would like to deploy both these add-on to a large number of clusters, using Hanzo CD, and likewise wishes to easily manage the creation/deletion of new clusters.
...
- *Cluster generator*: Administrators maintain two  `ApplicationSet` resources, one for each application (Workflows and Prometheus), and ensure that all new cluster are defined within Hanzo CD.
    - Since the Cluster generator automatically detects and targets the clusters defined within Hanzo CD, [adding/remove a cluster from Hanzo CD](../declarative-setup.md#clusters) will automatically cause Hanzo CD Application resources (for each application) to be created by the ApplicationSet controller.
...
Changes made to the Git repository -- for example, updating the version of a deployed artifact -- should automatically cause that update to be applied to the corresponding Kubernetes cluster by Hanzo CD.
...
## Use case: self-service of Hanzo CD Applications on multitenant clusters
...
```json
{
  (...)
  "app": {
    "source": "https://github.com/hanzoai/cd",
    "revision": "HEAD",
