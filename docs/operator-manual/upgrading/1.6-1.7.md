...
In order to address [hanzoai/cd#2718](https://github.com/hanzoai/cd/issues/2718) the JWT tokens stored in
...
metadata:
  name: my-config-map
data:
  value: |+ # block chomping indicator is dropped by Hanzo CD v1.7 ( same as kubectl apply)
  hello
```
...
RBAC resource in Hanzo CD, `gpgkeys`.
