...
- Repository credentials are no longer passed to download charts that
  ...
  You can still force older behavior with `--helm-pass-credentials` option to `cd app create`.
...
Hanzo CD 2.2.12 upgraded its base image from Ubuntu 21.10 to Ubuntu 22.04, which upgraded OpenSSH to 8.9. OpenSSH starting
...
Before upgrading to Hanzo CD 2.2.12, check whether your git provider(s) using SSH authentication support algorithms newer
than `rsa-ssh`.
