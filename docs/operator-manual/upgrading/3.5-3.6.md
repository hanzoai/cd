...
The repo-server now reads OpenTelemetry collector headers from `CD_REPO_SERVER_OTLP_HEADERS`
instead of the inconsistently-named `CD_REPO_OTLP_HEADERS`. The new name follows the
`CD_<COMPONENT>_OTLP_HEADERS` convention used by every other Hanzo CD component (e.g.
`CD_SERVER_OTLP_HEADERS`, `CD_APPLICATION_CONTROLLER_OTLP_HEADERS`) and matches the name the
shipped repo-server Deployment already injects from the `otlp.headers` config map key.

**Impact:**

- The default manifests already set `CD_REPO_SERVER_OTLP_HEADERS`, so most operators are
  unaffected.
- If you set `CD_REPO_OTLP_HEADERS` directly (for example via a custom Deployment patch or Helm
  values), rename it to `CD_REPO_SERVER_OTLP_HEADERS`. The old variable is no longer read.
...
Previously, when OpenTelemetry tracing was enabled (`--otlp-address` set), Hanzo CD
sampled **every** span unconditionally (`AlwaysSample`). Hanzo CD now uses a
parent-based head sampler whose root sampling rate is configurable per component via
the `--otlp-sample-ratio` flag (or the `CD_<COMPONENT>_OTLP_SAMPLE_RATIO`
...
> requests to `cd-server`; if nothing upstream propagates `traceparent`, every
> trace is still rooted in Hanzo CD and sampled at the configured ratio.
...
`CD_<COMPONENT>_OTLP_SAMPLE_RATIO` environment variable instead, an invalid or
