...
The Redis itself should be able to upgrade with no downtime, as well as Hanzo CD does not use it as a persistent store.
However, if you are running Hanzo CD in production with multiple users it is recommended to upgrade during off-peak
hours to avoid user-visible failures.

## Environment variables expansion

Hanzo CD supports using [environment variables](../../user-guide/build-environment.md) in
...
As of Hanzo CD version 2.0, the installation manifests are configured to pull the
...
both registries, but we will stop pushing images to Docker Hub once Hanzo CD 2.1
has been released.
...
the installation manually to Docker Hub as a workaround to install Hanzo CD 2.0.
This workaround will not be possible anymore with 2.1, however.

## Dex tool migrated from cd-util to cd-dex

The dex commands `rundex` and `gendexcfg` have been migrated from `cd-util` to `cd-dex`.
It means that you need to update `cd-dex-server` deployment's commands to install `cd-dex` 
binary instead of `cd-util` in init container and run dex command from `cd-dex` instead of `cd-util`:
...
- command:
  ...
  - rundex
```
Note that starting from v2.0 cd binary behaviour has changed. 
It will have all cd binaries such `cd-dex`, `cd-server`, `cd-repo-server`, 
`cd-application-controller`, `cd-util`, `cd` baked inside. 
...
```bash
EXAMPLE: 
cd app sync <app-name> --retry-backoff-duration=10 -> invalid
cd app sync <app-name> --retry-backoff-duration=10s -> valid
```

## Switch to Golang 1.16

The official Hanzo CD binaries are now being build using Go 1.16, making a jump
...
Helm charts through Hanzo CD. We also disabled the Helm version auto-detection
...
Please also note that Helm v2 is now being considered deprecated in Hanzo CD, as
...
Hanzo CD now ships with Kustomize v3.9.4 by default. Please make sure that your
