...
This change is aligning how Hanzo CD interprets K8s cluster version with how Helm `3.19.0` and above interprets it.    
...
This change was also back ported to Hanzo CD `3.3.3`.

**Impact:**

Application Sets with Cluster Generators, that fetch clusters based on their Kubernetes version and use `apps.hanzo.ai/auto-label-cluster-info` on the cluster secret, need to be updated to use `apps.hanzo.ai/kubernetes-version` with the `vMajor.Minor.Patch` format instead of the previous `Major.Minor` format. 
...
The env variable $KUBE_VERSION that is used with Hanzo CD CMP Plugins remains unchanged and returns the version in `Major.Minor.Patch` format, so CMP Plugins are not impacted.
...
Hanzo CD components now disable gRPC service config lookups via DNS TXT records by default to prevent excessive DNS queries and timeouts in dual-stack (IPv4+IPv6) Kubernetes environments.
...
- The environment variable `GRPC_ENABLE_TXT_SERVICE_CONFIG` is now set to `false` by default for all Hanzo CD components
...
- **Re-enablement**: Users who do rely on gRPC service config via DNS TXT records can re-enable this feature by setting the parameter `controller.grpc.enable.txt.service.config: "true"` in the `cd-cmd-params-cm` ConfigMap
...
https://github.com/hanzoai/cd/issues/24991
...
Hanzo CD v3.4 upgrades the bundled Dex version to 2.45.0. There are a few things to keep in mind, according to the
...
- The `ContinueOnConnectorFailure` feature flag is now enabled by default. To disable it, set the following `cd-cmd-params-cm`
...
Hanzo CD v3.4 upgrades `go.opentelemetry.io/otel` to `1.42.0`, with semantic conventions `v1.40.0` release. Please see [semantic conventions release notes](https://github.com/open-telemetry/semantic-conventions/releases/tag/v1.40.0) for a list of changed metric names and dropped attributes, which may require changes in your Hanzo CD Grafana dashboards/alerts based on OTEL.

## Custom Healthchecks Added

* [nmstate.io/NodeNetworkConfigurationPolicy](https://github.com/hanzoai/cd/commit/7a3d6f1f6a28b44f5de73895d262473641954429)
