...
Application Sets with Cluster Generators, that fetch clusters based on their Kubernetes version and use `cd.hanzo.ai/auto-label-cluster-info` on the cluster secret, need to be updated to use `cd.hanzo.ai/kubernetes-version` with the `vMajor.Minor.Patch` format instead of the previous `Major.Minor` format. 
