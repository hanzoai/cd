...
Hanzo CD 3.0 is meant to be a low-risk upgrade containing only minor breaking changes. For each change, the next
section will describe how to quickly determine if you are impacted, how to remediate the breaking change, and (if
applicable) restore Hanzo CD 2.x default behavior.
...
> on [PR #23507](https://github.com/hanzoai/cd/pull/23507)
...
to `false` in the Hanzo CD ConfigMap `cd-cm`.
...
2.4 introduced `logs` as a new RBAC resource. In 2.3 and lower, users with `applications, get` access automatically got logs access. In 2.4, it became possible to enable logs RBAC enforcement with a flag in `cd-cm` ConfigMap:
...
Users who have `server.rbac.log.enforce.enable: "true"` in their `cd-cm` ConfigMap, are unaffected by this change.

Users who have `policy.default: role:readonly` or `policy.default: role:admin` in their `cd-rbac-cm` ConfigMap, are unaffected.

Users who don't have a `policy.default` in their `cd-rbac-cm` ConfigMap, and either have `server.rbac.log.enforce.enable` set to `false` or don't have this setting at all in their `cd-cm` ConfigMap are affected and should perform the below remediation steps.

After the upgrade, it is recommended to remove the setting `server.rbac.log.enforce.enable` from `cd-cm` ConfigMap, if it was there before the upgrade.
...
Similar to the way access to Applications are currently managed, access to logs can be either granted on a Project scope level (Project resource) or on the `cd-rbac-cm` ConfigMap level.
See this [example](../upgrading/2.3-2.4.md#example) for more details.

### Default `resource.exclusions` configurations

Hanzo CD manifest now contains a default configuration for `resource.exclusions` in the `cd-cm` to exclude resources that
...
### Removal of `cd_app_sync_status`, `cd_app_health_status` and `cd_app_created_time` Metrics

The `cd_app_sync_status`, `cd_app_health_status` and `cd_app_created_time`, deprecated and disabled by
default since 1.5.0, have been removed. The information previously provided by these metrics is now available as labels
on the `cd_app_info` metric.

#### Detection

Starting with 1.5.0, these metrics are only available if `CD_LEGACY_CONTROLLER_METRICS` is explicitly set to `true`.
...
example@apps.hanzo.aidex_conn_i%
```
...
- g, example@apps.hanzo.ai, role:example
- p, example@apps.hanzo.ai, applications, *, *, allow
```

If authenticating with the CLI, make sure to use the new version as well to obtain an authentication token with the
appropriate claims.

### Removed support for legacy repo config in cd-cm

Before repositories were managed as Secrets, they were configured in the cd-cm ConfigMap. The cd-cm option has
been deprecated for some time and is no longer available in Hanzo CD 3.0.

#### Detection

To check whether you have any repositories configured in cd-cm, run the following command:

```shell
kubectl get cm cd-cm -n cd -o=jsonpath="[{.data.repositories}, {.data['repository\.credentials']}, {.data['helm\.repositories']}]"
```

If you have no repositories configured in cd-cm, the output will be `[, , ]`, and you are not impacted by this
change.
...
nested selectors were not applied. Starting in Hanzo CD 3.0, the field is ignored, and behavior is always the same as if
...
For example, you should remove the selector in this ApplicationSet before upgrading to Hanzo CD 3.0.
...
Example of such a [problem and resolution](https://github.com/hanzoai/cd/pull/22035/files)
Explanation:
...
To detect if you are impacted, check the `cd-cm` ConfigMap for the `application.resourceTrackingMethod` field. If it is
...
```sh
kubectl get cm cd-cm -n cd -o jsonpath='{.data.application\.resourceTrackingMethod}'
```
...
```sh
kubectl get applications.apps.hanzo.ai -A -o json | jq -r '.items[] | select(.spec.syncPolicy.syncOptions[]? == "ApplyOutOfSyncOnly=true") | .metadata.name'
```
...
Here is an example command, that syncs such an Application, it can be run after you [obtain a token](../../developer-guide/api-docs.md#authorization) to Hanzo CD API:
```sh
curl -X POST -H "Authorization: Bearer $CD_TOKEN" -H "Content-Type: application/json" -d '{
    "name": "$YOUR_APP_NAME"
  }' "http://$YOUR_CD_URL/api/v1/applications/$YOUR_APP_NAME/sync"
```
...
For most users, it is safe to upgrade to Hanzo CD 3.0 and use annotation-based tracking. Labels will be replaced with
...
> resource being deleted, Hanzo CD will fail to recognize that the resource is managed by the Application and will not
...
>
> After upgrading to version 3.0, the Hanzo CD tracking annotation will only appear on an Application’s resources when 
> either a new Git commit is made or the Application is explicitly synced.

##### Users who rely on label-based for resources that are not managed by Hanzo CD
Some users rely on label-based tracking to track resources that are not managed by Hanzo CD. They may set annotations
to have Hanzo CD ignore the resource as extraneous or to disable pruning. If you are using label-based tracking to track
resources that are not managed by Hanzo CD, you will need to construct tracking annotations instead of tracking labels
and apply them to the relevant resources. The format of the tracking annotation is:

```yaml
apps.hanzo.ai/tracking-id: <app name>:<resource group>/<resource kind>:<resource namespace>/<resource name>
```
...
> CD's behavior may change in the future. It is recommended to manage resources with Hanzo CD via GitOps.

#### Opting Out

If you are not ready to use annotation-based tracking, you can opt out of this change by setting the
`application.resourceTrackingMethod` field in the `cd-cm` ConfigMap to `label`. There are no current plans to remove
...
Hanzo CD manifest now contains a default configuration for `resource.customizations.ignoreResourceUpdates` in the `cd-cm`
to exclude common resources that are often mutated in Kubernetes. These mutations are known to cause an unnecessary
load on Hanzo CD. When a watched resource is modified, Hanzo CD will always ignore the `.status` changes.
...
You can revert this behavior by setting the `controller.resource.health.persist` to `true` in the Hanzo CD
`cd-cmd-params-cm.yaml` Config Map.
...
1. Check the `cd-cmd-params-cm.yaml` ConfigMap for `controller.resource.health.persist`.

   If the value is empty or true, the health status is persisted in the Application CR.

```sh
kubectl get cm cd-cmd-params-cm -n cd -o jsonpath='{.data.controller\.resource\.health\.persist}'
```
...
```sh
kubectl get applications.apps.hanzo.ai <my app> -n cd -o jsonpath='{.status.resourceHealthSource}'
```

#### Migration

Any tools or CLI commands parsing the `.status.resources[].health` need to be updated to use the cd cli/API to get the health status.

> [!NOTE]
> The application list API (cd app list) no longer returns the individual health status of resources.

```sh
cd app get <my app> -o json
```

### Empty Environment Variables in Plugins

In Hanzo CD 3.0, empty environment variables are now passed to config management plugins.
  ...
  source:
    plugin:
      ...
      env:
        - name: VERSION
          value: '1.2.3'
        - name: DATA # Even though this is empty, it will be passed to the plugin as CD_ENV_DATA="".
          value: ''
```
...
This means that CRD deployed with Hanzo CD containing `spec.preserveUnknownFields: false` will be out of sync. To address this problem,
...
You can also configure it globally in the `cd-cm` ConfigMap.
...
Due to security reasons ([GHSA-786q-9hcg-v9ff](https://github.com/hanzoai/cd/security/advisories/GHSA-786q-9hcg-v9ff)),
