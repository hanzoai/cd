...
Hanzo CD 2.7 upgrades Sprig templating specifically within Hanzo CD notifications to v3. That upgrade includes an upgrade of [Masterminds/semver](https://github.com/Masterminds/semver/releases) to v3.
...
[3]: https://github.com/hanzoai/cd/pull/12707
...
Hanzo CD now supports the `helm.sh/resource-policy` annotation to control the deletion of resources. The behavior is the same as the behavior of
`apps.hanzo.ai/sync-options: Delete=false` annotation: if the annotation is present and set to `keep`, the resource will not be deleted
when the application is deleted.

## Check your Kustomize patches for `--redis` changes

Starting in Hanzo CD 2.7, the install manifests no longer pass the Redis server name via `--redis`. 
...
field in the cd-cmd-params-cm ConfigMap. That value will be passed to the necessary components via `valueFrom` 
environment variables.

## `cd applicationset` CLI incompatibilities for ApplicationSets with list generators

If you are running Hanzo CD v2.7.0-2.7.2 server-side, then CLI versions outside that range will incorrectly handle list
generators. That is because the gRPC interface for those versions used the `elements` field number for the new
`elementsYaml` field.

If you are running the Hanzo CD CLI versions v2.7.0-2.7.2 with a server-side version of v2.7.3 or later, then the CLI
