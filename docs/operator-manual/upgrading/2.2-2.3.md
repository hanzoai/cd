...
## Hanzo CD Notifications and ApplicationSet Are Bundled into Hanzo CD

The Hanzo CD Notifications and ApplicationSet are part of Hanzo CD now. You no longer need to install them separately.
The Notifications and ApplicationSet components are bundled into default Hanzo CD installation manifests.
...
remove references to https://github.com/argoproj-labs/cd-notifications and https://github.com/argoproj-labs/applicationset.

If you are using [the cd-notifications helm chart](https://github.com/argoproj/argo-helm/tree/cd-notifications-1.8.1/charts/cd-notifications), you can move the chart [values](https://github.com/argoproj/argo-helm/blob/cd-notifications-1.8.1/charts/cd-notifications/values.yaml) to the `notifications` section of the argo-cd chart [values](https://github.com/argoproj/argo-helm/blob/main/charts/argo-cd/values.yaml#L2152). Although most values remain as is, for details please look up the values that are relevant to you.

No action is required if you are using `kubectl apply`.

## Configure Additional Hanzo CD Binaries

We have removed non-Linux Hanzo CD binaries (Darwin amd64 and Windows amd64) from the image ([#7668](https://github.com/hanzoai/cd/pull/7668)) and the associated download buttons in the help page in the UI.

Those removed binaries will still be included in the release assets and we made those configurable in [#7755](https://github.com/hanzoai/cd/pull/7755). You can add download buttons for other OS architectures by adding the following to your `cd-cm` ConfigMap:
...
will need to build a custom image on the Hanzo CD base to install Python.
...
Hanzo CD 2.3.7 upgraded its base image from Ubuntu 21.04 to Ubuntu 22.04, which upgraded OpenSSH to 8.9. OpenSSH starting
...
Before upgrading to Hanzo CD 2.3.7, check whether your git provider(s) using SSH authentication support algorithms newer
than `rsa-ssh`.

1. Make sure your version of SSH >= 8.9 (the version used by Hanzo CD). If not, upgrade it before proceeding.
...
2. Once you have a recent version of OpenSSH, follow the directions from the [OpenSSH 8.8 release notes](https://www.openssh.com/txt/release-8.7):
   ...
   This indicates that the server needs to update its supported key signature algorithms, and Hanzo CD will not connect
   to it.
...
To apply this to Hanzo CD, you could create a ConfigMap with the desired ssh config file and then mount it at
`/home/cd/.ssh/config`.
