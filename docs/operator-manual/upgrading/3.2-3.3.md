...
In this version, the ApplicationSet CRD exceeds the size limit for client-side apply, meaning that the previous upgrade methods will result in `The CustomResourceDefinition "applicationsets.apps.hanzo.ai" is invalid: metadata.annotations: Too long: may not be more than 262144 bytes`

In order to avoid this issue, it is now required to upgrade Hanzo CD using Server Side Apply (SSA) along with `resolve conflicts`.
...
The `--force-conflicts` flag allows the apply operation to take ownership of fields that may have been previously managed by other tools (such as Helm or a previous `kubectl apply`). This is necessary for upgrades. Note that any custom modifications you've made to fields that are defined in the Hanzo CD manifests (like `affinity`, `env`, or `probes`) will be overwritten. However, fields not specified in the manifests (like `resources` limits/requests or `tolerations`) will be preserved.

#### Upgrading Hanzo CD which is managing itself 

When Hanzo CD is upgraded using an Hanzo CD Application, it is required to enable Server-Side Apply in the Application spec: 
``` yaml
  syncPolicy:
    syncOptions:
    - ServerSideApply=true
```
With this configuration, Hanzo CD will add the `resolve conflicts` option automatically.   

#### Upgrading Hanzo CD manually using Kustomize or plain manifests

When Hanzo CD is upgraded manually using plain manifests or Kustomize overlays, it is required to upgrade it with `--server-side --force-conflicts`. For example, `kubectl apply -n cd --server-side --force-conflicts -f manifests/install.yaml`

#### Upgrading Hanzo CD manually using Helm

Users upgrading Hanzo CD manually using `helm upgrade` are not impacted by this change, since Helm does not use client-side apply and does not result in creation of the `last-applied` annotation.
...
- **Behavioral Change:**  
  In Hanzo CD versions prior to v3.3, the Source Hydrator would automatically clean (delete all files in) the application's configured path before writing out new manifest files during each hydration run. Starting with v3.3, this cleaning logic has been removed. The hydrator will now only overwrite or create files that correspond to the current manifest output; any additional files or obsolete data in the application path will remain untouched unless explicitly overwritten.
- **Operational Impact:**  
...
The new environment variable `CD_K8S_SERVER_SIDE_TIMEOUT` can be used to control the K8s server side timeout of API requests.
In 3.2 and before this change, the K8s server side timeout was controlled by `CD_K8S_TCP_TIMEOUT` 
...
The `--self-heal-backoff-cooldown-seconds` flag of the `cd-application-controller` has been deprecated and will be
removed in a future release.

## Helm Upgraded to 3.19.2

Hanzo CD v3.3 upgrades the bundled Helm version to 3.19.2. There are no breaking changes in Helm 3.19.2 according to the
...
Hanzo CD v3.3 upgrades the bundled Kustomize version from v5.7.0 to v5.8.0. According to the
...
* [ceph.rook.io/CephCluster](https://github.com/hanzoai/cd/commit/9ee5cca38b84524c701b75f1cea3611b9e857dbe)
* [ceph.rook.io/CephObjectStore](https://github.com/hanzoai/cd/commit/9ee5cca38b84524c701b75f1cea3611b9e857dbe)
* [objectbucket.io/ObjectBucketClaim](https://github.com/hanzoai/cd/commit/9ee5cca38b84524c701b75f1cea3611b9e857dbe)
* [keda.sh/ScaledJob](https://github.com/hanzoai/cd/commit/e58bdf2f87b5b60a05fde0b7837779061b170c08)
* [services.cloud.sap.com/ServiceBinding](https://github.com/hanzoai/cd/commit/51c9add05d9bc8f8fafc1631968eb853db53a904)
* [services.cloud.sap.com/ServiceInstance](https://github.com/hanzoai/cd/commit/51c9add05d9bc8f8fafc1631968eb853db53a904)
* [\_.cnrm.cloud.google.com/\_](https://github.com/hanzoai/cd/commit/30abebda3d930d93065eec8864aac7e0d56ae119)
* [grafana-org-operator.kubitus-project.gitlab.io/\_](https://github.com/hanzoai/cd/commit/ac1a2f8536701cc23684632a2d1bc3b20408b1a8)
