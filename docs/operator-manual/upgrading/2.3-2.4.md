...
The time has come to remove it from the Hanzo CD.
...
Helm 2 support was preserved in the Hanzo CD. We feel that Helm 3 is stable, and it is time to drop Helm 2 support.
...
Hanzo CD 2.4 upgraded its base image from Ubuntu 20.04 to Ubuntu 22.04, which upgraded OpenSSH to 8.9. OpenSSH starting
...
Before upgrading to Hanzo CD 2.4, check whether your git provider(s) using SSH authentication support algorithms newer
than `rsa-ssh`.
...
> [was made](https://github.com/hanzoai/cd/issues/10551#issuecomment-1242303457) to avoid breaking logs access 
> under [Project Roles](../../user-guide/projects.md#project-roles), which do not provide a mechanism to grant `logs`
> resource access.

To enabled logs RBAC enforcement, add this to your cd-cm ConfigMap:
...
As a security enhancement, the cd-repo-server Deployment uses its own Service Account instead of `default`.
...
Hanzo CD 2.4. Here is an example of user-supplied environment variables in the `plugin` section of an Application spec:
...
Going forward, all user-supplied environment variables will be prefixed with `CD_ENV_` before being sent to the 
...
If you use a third-party plugin which does not explicitly advertise Hanzo CD 2.4 support, it might not handle the 
prefixed environment variables. Open an issue with the plugin's authors and confirm support before upgrading to Hanzo CD 
2.4.
...
cd-cm plugins will continue to receive environment variables from the main repo-server container.
