...
Hanzo CD Notifications continuously monitors Hanzo CD applications and provides a flexible way to notify
...
well as notification content. Hanzo CD Notifications includes the [catalog](catalog.md) of useful triggers and templates.
...
A common installation method for Hanzo CD Notifications is to install it in a dedicated namespace to manage a whole cluster. In this case, the administrator is the only
...
for their Hanzo CD applications. For example, the end-user can configure notifications for their Hanzo CD application in the namespace where they have access to and their Hanzo CD application is running in.
...
In order to enable this feature, the Hanzo CD administrator must reconfigure the cd-notification-controller workloads to add  `--application-namespaces` and `--self-service-notification-enabled` parameters to the container's startup command.
`--application-namespaces` controls the list of namespaces that Hanzo CD applications are in. `--self-service-notification-enabled` turns on this feature.

The startup parameters for both can also be conveniently set up and kept in sync by specifying
the `application.namespaces` and `notificationscontroller.selfservice.enabled` in the cd-cmd-params-cm ConfigMap instead of changing the manifests for the respective workloads. For example:
...
To use this feature, you can deploy configmap named `cd-notifications-cm` and possibly a secret `cd-notifications-secret` in the namespace where the Hanzo CD application lives.
...
the configuration located in the same namespace where the Hanzo CD application is at.
...
The following two resources are deployed in the namespace where the Hanzo CD application lives.
```yaml
apiVersion: v1
kind: ConfigMap
metadata:
