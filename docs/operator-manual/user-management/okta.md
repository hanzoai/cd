...
>
> The Okta app's **Group Attribute Statements** regex will be used later to map Okta groups to Hanzo CD RBAC roles.
...
1. Copy the Hanzo CD URL to the `cd-cm` in the data.url
...
1. Download the CA certificate to use in the `cd-cm` configuration.
    ...
    * If you are using the ca field and storing the CA certificate separately as a secret, you will need to mount the secret to the `dex` container in the `cd-dex-server` Deployment.
    * ![Okta SAML App 4](../../assets/saml-4.png)
1. Edit the `cd-cm` and configure the `data.dex.config` section:
...
dex.config: |
  ...
  - type: saml
    ...
    config:
      ...
      ca: /path/to/ca.pem
      redirectURI: https://ui.cd.yourorganization.net/api/dex/callback
      usernameAttr: email
      emailAttr: email
      groupsAttr: group
```
...
### Private deployment
It is possible to setup Okta SSO with a private Hanzo CD installation, where the Okta callback URL is the only publicly exposed endpoint.
The settings are largely the same with a few changes in the Okta app configuration and the `data.dex.config` section of the `cd-cm` ConfigMap.

Using this deployment model, the user connects to the private Hanzo CD UI and the Okta authentication flow seamlessly redirects back to the private UI URL.

Often this public endpoint is exposed through an [Ingress object](../ingress.md#private-hanzo-cd-ui-with-multiple-ingress-objects-and-byo-certificate).
...
1. Update the `data.dex.config` section of the `cd-cm` ConfigMap with the external endpoint reference.
...
### Connect Okta Groups to Hanzo CD Roles
Hanzo CD is aware of user memberships of Okta groups that match the *Group Attribute Statements* regex.
The example above uses the `cd-*` regex, so Hanzo CD would be aware of a group named `cd-admins`.

Modify the `cd-rbac-cm` ConfigMap to connect the `cd-admins` Okta group to the builtin Hanzo CD `admin` role.
<!-- markdownlint-disable MD046 -->
```yaml
apiVersion: v1
kind: ConfigMap
...
1. Update the following:
    ...
    1. `Sign-in redirect URLs`: Add `https://cd.example.com/auth/callback`; replacing `cd.example.com` with your Hanzo CD web interface URL.
    1. `Sign-out redirect URIs`: Add `https://cd.example.com`; substituting the correct domain name as above. 
...
1. Click `Add Authorization Server`, and assign it a name and a description. The `Audience` should match your Hanzo CD URL - `https://cd.example.com`
...
1. Click `Claims > Add Claim`:
    ...
    1. Add a filter that will match the Okta groups you want passed on to Hanzo CD; for example `Regex: cd-.*`.
...
In order to login with the CLI `cd login https://cd.example.com --sso`, Okta requires a separate dedicated App Integration:
...
1. Update the following:
    ...
    1. Either assign groups, or choose to skip this step for now.
    1. Leave the rest of the options as-is, and save the integration.
    1. Copy the `Client ID` from the newly created app; `cliClientID: <Client ID>` will be used in your `cd-cm` ConfigMap.
...
Finally, configure Hanzo CD itself. Edit the `cd-cm` configmap:
