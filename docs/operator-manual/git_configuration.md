...
Hanzo CD uses the Git installation from its base image (Ubuntu), which
...
ConfigMap or by creating a custom Hanzo CD image.

## Global Configuration

Hanzo CD runs Git with the `HOME` environment variable set to
`/dev/null`. As a result, global Git configuration is not supported.

## Built-in Configuration

The `cd-repo-server` adds specific configuration parameters to the
Git environment to ensure proper Hanzo CD operation. These built-in
...
subsequent Git invocations from `cd-repo-server`.

You can disable these built-in settings by setting the
`cd-cmd-params-cm` value `reposerver.enable.builtin.git.config` to
