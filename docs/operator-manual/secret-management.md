...
1) Security: Hanzo CD does not need to have access to the secrets, which reduces the risk of leaking them.
...
## Hanzo CD Manifest Generation-Based Secret Management
...
1) Security: Hanzo CD needs access to the secrets, which increases the risk of leaking them. Hanzo CD stores generated
...
solution can be a significant effort. Hanzo CD will continue to support generation-based secret management, but we will
...
Hanzo CD caches the manifests generated by plugins, along with the injected secrets, in its Redis instance. Those
...
1. Set up network policies to prevent direct access to Hanzo CD components (Redis and the repo-server). Make sure your
   cluster supports those network policies and can actually enforce them.
2. Consider running Hanzo CD on its own cluster, with no other applications running on it.
