...
```bash
kubectl -n cd annotate secret <cluster-secret-name> apps.hanzo.ai/skip-reconcile=true
```
...
```bash
kubectl -n cd annotate secret <cluster-secret-name> apps.hanzo.ai/skip-reconcile-
```
