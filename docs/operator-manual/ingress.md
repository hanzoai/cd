...
Hanzo CD API server runs both a gRPC server (used by the CLI), as well as an HTTP/HTTPS server (used by the UI).
Both protocols are exposed by the cd-server service object on the following ports:
...
The API server should be run with TLS disabled. Edit the `cd-server` deployment to add the `--insecure` flag to the cd-server command, or simply set `server.insecure: "true"` in the `cd-cmd-params-cm` ConfigMap [as described here](server-commands/additional-configuration-method.md). Given the `cd` CLI includes the port number in the request `host` header, two Mappings are required.
...
spec:
  ...
  host: cd.example.com:443
  prefix: /
  service: cd-server:80
  regex_headers:
    Content-Type: "^application/grpc.*$"
  grpc: true
```

Login with the `cd` CLI:
...
Example of `cd-cmd-params-cm` configmap
```yaml
apiVersion: v1
kind: ConfigMap
metadata:
  ...
  namespace: cd
  labels:
    app.kubernetes.io/name: cd-cmd-params-cm
...
The Hanzo CD API server should be run with TLS disabled. Edit the `cd-server` Deployment to add the `--insecure` flag to the cd-server container command, or simply set `server.insecure: "true"` in the `cd-cmd-params-cm` ConfigMap [as described here](server-commands/additional-configuration-method.md).
...
This provides the opportunity to deploy the Hanzo CD UI privately but still allow for SSO callbacks to succeed.

### Private Hanzo CD UI with  Multiple Ingress Objects and BYO Certificate
...
spec:
  rules:
  - host: grpc-internal.path.to.cd.io
    http:
      paths:
      - path: /
        pathType: Prefix
...
kind: Ingress
metadata:
  name: cd-server-external-callback-http
  annotations:
    kubernetes.io/ingress.class: contour-external
    ingress.kubernetes.io/force-ssl-redirect: "true"
spec:
  rules:
  - host: external.path.to.cd.io
    http:
      paths:
  ...
  tls:
  - hosts:
    - external.path.to.cd.io
    secretName: your-certificate-name
```

The cd-server Service needs to be annotated with `projectcontour.io/upstream-protocol.h2c: "https,443"` to wire up the gRPC protocol proxying.

The API server should then be run with TLS disabled. Edit the `cd-server` deployment to add the
`--insecure` flag to the cd-server command, or simply set `server.insecure: "true"` in the `cd-cmd-params-cm` ConfigMap [as described here](server-commands/additional-configuration-method.md).
...
Hanzo CD serves multiple protocols (gRPC/HTTPS) on the same port (443), this provides a
challenge when attempting to define a single nginx ingress object and rule for the cd-service,
...
In order to expose the Hanzo CD API server with a single ingress rule and hostname, the
...
must be used to passthrough TLS connections and terminate TLS at the Hanzo CD API server.

```yaml
apiVersion: networking.k8s.io/v1
kind: Ingress
metadata:
  name: cd-server-ingress
  namespace: cd
  annotations:
...
The above rule terminates TLS at the Hanzo CD API server, which detects the protocol being used,
...
spec:
  ...
  - hosts:
    - cd.example.com
    secretName: cd-server-tls # as expected by cd-server
```
...
Each ingress will be for a different domain (`cd.example.com` and `grpc.cd.example.com`). This requires that the Ingress resources use different TLS `secretName`s to avoid unexpected behavior.
      ...
      - path: /
        ...
        backend:
          service:
            ...
            port:
              name: https
    host: grpc.cd.example.com
  tls:
  - hosts:
    - grpc.cd.example.com
    secretName: cd-ingress-grpc
```
...
To properly support Hanzo CD, this Ingress Controller must be configured with HTTP/2 support, via its [ConfigMap](https://docs.nginx.com/nginx-ingress-controller/configuration/global-configuration/configmap-resource/#listeners).
...
metadata:
  ...
  namespace: cd
  annotations:
    nginx.org/grpc-services: cd-server
spec:
  ingressClassName: f5-nginx
  rules:
  - http:
...
Run the API server with TLS disabled. Edit the `cd-server` deployment to add the `--insecure` flag to the cd-server command or set `server.insecure: "true"` in the `cd-cmd-params-cm` ConfigMap [as described here](server-commands/additional-configuration-method.md).
...
spec:
  ...
  routes:
    - kind: Rule
      match: Host(`cd.example.com`)
      priority: 10
      services:
        - name: cd-server
          port: 80
    - kind: Rule
      match: Host(`cd.example.com`) && Header(`Content-Type`, `application/grpc`)
      priority: 11
      services:
        - name: cd-server
          port: 80
...
When using an ALB, you'll want to create a second service for cd-server. This is necessary because we need to tell the ALB to send the GRPC traffic to a different target group than the UI traffic, since the backend protocol is HTTP2 instead of HTTP1.
...
spec:
  ...
  - name: "443"
    ...
    targetPort: 8080
  selector:
    app.kubernetes.io/name: cd-server
...
Also note that we can configure the health check to return the gRPC health status code `OK - 0` from the cd-server by setting the health check path to `/grpc.health.v1.Health/Check`. By default, the ALB health check for gRPC returns the status code `UNIMPLEMENTED - 12` on health check path `/AWS.ALB/healthcheck`.
...
You can put Hanzo CD behind Istio using the following configuration. This example serves Hanzo CD behind Istio and uses a subpath (for example, `/cd`).
...
Install Hanzo CD (there should be only the three YAML files defined above in the current directory):

```bash
kubectl apply -k ./ -n cd --wait=true
```

Be sure you create secret for Istio ( in our case secretname is cd-server-tls on cd Namespace). After that we create Istio Resources
...
You can now browse to `http://<IP>/cd` (it will be redirected to HTTPS).
...
Edit the `--insecure` flag in the `cd-server` command of the cd-server deployment, or simply set `server.insecure: "true"` in the `cd-cmd-params-cm` ConfigMap [as described here](server-commands/additional-configuration-method.md).

### Creating a service

Now you need an externally accessible service. This is practically the same as the internal service Hanzo CD has, but with Google Cloud annotations. Note that this service is annotated to use a [Network Endpoint Group](https://cloud.google.com/load-balancing/docs/negs) (NEG) to allow your load balancer to send traffic directly to your pods without using kube-proxy, so remove the `neg` annotation if that's not what you want.
...
metadata:
  ...
  annotations:
    ...
    cloud.google.com/backend-config: '{"ports": {"http":"cd-backend-config"}}'
spec:
  type: ClusterIP
  ports:
  - name: http
...
See that previous service referencing a backend config called `cd-backend-config`? So let's deploy it using this YAML:

```yaml
apiVersion: cloud.google.com/v1
kind: BackendConfig
metadata:
  name: cd-backend-config
  namespace: cd
spec:
  healthCheck:
    checkIntervalSec: 30
...
```
kubectl -n cd create secret tls secret-yourdomain-com \
...
metadata:
  ...
  namespace: cd
  annotations:
    networking.gke.io/v1beta1.FrontendConfig: cd-frontend-config
spec:
  tls:
...
As you may know already, it can take some minutes to deploy the load balancer and become ready to accept connections. Once it's ready, get the public IP address for your Load Balancer, go to your DNS server (Google or third party) and point your domain or subdomain (i.e. cd.example.com) to that IP address.
...
Hanzo CD endpoints may be protected by one or more reverse proxies layers, in that case, you can provide additional headers through the `cd` CLI `--header` parameter to authenticate through those layers.

```shell
$ cd login <host>:<port> --header 'x-token1:foo' --header 'x-token2:bar' # can be repeated multiple times
$ cd login <host>:<port> --header 'x-token1:foo,x-token2:bar' # headers can also be comma separated
```
...
This section discusses using Gateway API to expose the Hanzo CD server in various TLS configurations,
...
#### Securing traffic between Hanzo CD and the gateway

If your security requirements allow it, the Hanzo CD API server can be run with TLS disabled: pass the `--insecure` flag to the `cd-server` command,
or set `server.insecure: "true"` in the `cd-cmd-params-cm` ConfigMap [as described here](server-commands/additional-configuration-method.md).

It is also possible to keep TLS enabled, encrypting traffic between the gateway and the Hanzo CD API server, by using a [BackendTLSPolicy](https://gateway-api.sigs.k8s.io/api-types/backendtlspolicy/).
...
spec:
  ...
  validation:
    caCertificateRefs:
      - kind: ConfigMap
        name: cd-server-ca-cert
        group: ""
    hostname: cd-server.cd.svc.cluster.local
```
...
The `cd` CLI operates at full capability when using gRPC over HTTP/2 to communicate with the API server, falling back to HTTP/1.1. (`--grpc-web` flag).

gRPC can be configured using a `GRPCRoute`, and HTTP/2 requested as the application protocol on the `cd-server` service:
...
And in Hanzo CD's `values.yaml` (or [directly](https://kubernetes.io/docs/concepts/services-networking/service/#application-protocol) in the service manifest):
```
server:
  service:
...
TLS can also be configured to terminate at the Hanzo CD API server.
...
and found by the Hanzo CD server in the `cd-server-tls` secret.
