...
> [!TIP]
> Looking for general TLS setup across Hanzo CD components? See [TLS configuration](./tls.md).

Hanzo CD supports mutual TLS (mTLS) between the `cd-server` (API server), `cd-application-controller`, `cd-applicationset-controller`, and the `cd-repo-server`. This ensures the repo-server only accepts connections from authorized clients that present a valid client certificate.
...
This is the simplest and recommended way to enable mTLS. A single client certificate and key are shared by all client components (`cd-server`, `cd-application-controller`, `cd-applicationset-controller`, and `cd-notifications-controller`). Hanzo CD auto-mounts the Secret into every relevant deployment — no manual volume or volumeMount configuration is required.

**Step 1 — Create the `cd-repo-server-mtls` Secret** in the `cd` namespace with the following keys:

```yaml
apiVersion: v1
kind: Secret
metadata:
  name: cd-repo-server-mtls
  namespace: cd
type: Opaque
data:
  # CA used by cd-repo-server to verify incoming client certificates (enables mTLS)
...
To enable mTLS on the `cd-repo-server`, provide a client CA certificate. When configured, the repo-server requires all clients to present a certificate signed by this CA.
...
The `cd-server`, `cd-application-controller`, and `cd-applicationset-controller` must be configured with a client certificate and key to connect to a repo-server that has mTLS enabled. The certificate must be signed by the same CA configured on the repo-server.
...
### cd-cmd-params-cm ConfigMap keys

These environment variables can also be set via the `cd-cmd-params-cm` ConfigMap, which is the recommended approach for Kubernetes deployments:
...
By default, the `cd-repo-server-mtls` Secret uses a **single shared key/cert pair** (`client.crt` / `client.key`) that is mounted into every client component (`cd-server`, `cd-application-controller`, `cd-applicationset-controller`, and `cd-notifications-controller`). All components therefore present the same client certificate to the repo-server.
...
Store all component certs in the single `cd-repo-server-mtls` Secret under different key names, then customize the volume `items` projection in each deployment to expose only the relevant cert.
...
```yaml
# Example patch for cd-server deployment
volumes:
- name: cd-repo-server-mtls
  secret:
    secretName: cd-repo-server-mtls
    items:
...
Create a separate Secret for each component. Each Secret follows the same format as `cd-repo-server-mtls` but contains only that component's cert.
...
kind: Secret
metadata:
  name: cd-repo-server-mtls-server
  namespace: cd
type: Opaque
data:
...
kind: Secret
metadata:
  name: cd-repo-server-mtls-controller
  namespace: cd
type: Opaque
data:
  client.crt: <BASE64_CONTROLLER_CLIENT_CERT_PEM>
  client.key: <BASE64_CONTROLLER_CLIENT_KEY_PEM>
# ... repeat for cd-applicationset-controller and cd-notifications-controller
```
...
- name: cd-repo-server-mtls-server
  secret:
    secretName: cd-repo-server-mtls-server
volumeMounts:
- name: cd-repo-server-mtls-server
...
> [!IMPORTANT]
> With Option B, the default `cd-repo-server-mtls` volume that Hanzo CD auto-mounts must be removed or overridden in each patched deployment, otherwise both volumes will compete for the same mount path.
...
3. Connections from `cd-server`/`cd-application-controller`/`cd-applicationset-controller` should succeed when configured with matching client cert/key.
