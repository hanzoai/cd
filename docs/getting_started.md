...
[https://github.com/hanzocd/example-apps.git](https://github.com/hanzocd/example-apps.git) to demonstrate how Hanzo CD works.
...
```bash
cd app create guestbook --repo https://github.com/hanzocd/example-apps.git --path guestbook --dest-server https://kubernetes.default.svc --dest-namespace default
```
...
Connect the [https://github.com/hanzocd/example-apps.git](https://github.com/hanzocd/example-apps.git) repo to Hanzo CD by setting repository url to the github repo url, leave revision as `HEAD`, and set the path to `guestbook`:
