...
interface PodViewProps {
    ...
    app: Application;
    nodeMenu?: (node: ResourceNode) => ReactNode;
    quickStarts?: (node: ResourceNode) => ReactNode;
}
    ...
    return (
        <DataLoader load={() => services.viewPreferences.getPreferences()}>
            {prefs => {
                ...
                const groups = processTree(podPrefs.sortMode, props.tree.hosts || [], props.tree, props.app, props.nodeMenu, props.quickStarts) || [];
                        ...
                        <div className='pod-view__settings'>
                            <div className='pod-view__settings__section'>
                                ...
                                <DropDownMenu
                                    ...
                                    items={menuItemsFor(['node', 'parentResource', 'topLevelResource'], prefs, ctx)}
                                />
                            </div>
                            {podPrefs.sortMode !== 'node' && (
                                <div className='pod-view__settings__section'>
                                    ...
                                    <DropDownMenu
                                        ...
                                        items={sortOrderItemsFor(['asc', 'desc'], prefs, ctx)}
                                    />
                                </div>
                            )}
                            {podPrefs.sortMode === 'node' && (
...
function sortOrderItemsFor(orders: SortOrder[], prefs: ViewPreferences, ctx: ContextApis & {history: History}): MenuItem[] {
    const podPrefs = prefs.appDetails.podView || ({} as PodViewPreferences);
...
function menuItemsFor(modes: PodGroupType[], prefs: ViewPreferences, ctx: ContextApis & {history: History}): MenuItem[] {
    const podPrefs = prefs.appDetails.podView || ({} as PodViewPreferences);
...
function processTree(
    ...
    app.status?.resources?.forEach(res => statusByKey.set(nodeKey(res), res));
    ...
    (tree.nodes || []).forEach((rnode: ResourceTreeNode) => {
        ...
        if (sortMode !== 'node') {
            ...
            groupRefs[rnode.uid] = {
                ...
                renderMenu: () => nodeMenu(rnode),
                renderQuickStarts: () => quickStarts(rnode)
            };
        }
    });
    (tree.nodes || []).forEach((rnode: ResourceTreeNode) => {
