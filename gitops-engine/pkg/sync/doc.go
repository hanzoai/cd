...
Hooks are regular Kubernetes resources that have `apps.hanzo.ai/hook` annotation:
...
The same resource hook might be executed in several sync phases:
	...
	metadata:
	  generateName: schema-migrate-
	  annotations:
	    apps.hanzo.ai/hook: PreSync,PostSync

Hooks can be deleted in an automatic fashion using the annotation: apps.hanzo.ai/hook-delete-policy.
...
that runs before all other resources. The `apps.hanzo.ai/sync-wave` annotation assign resource to a wave:
...
annotation 'apps.hanzo.ai/sync-options'. Following sync options are supported:
