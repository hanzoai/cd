...
func TestPullRequestGithubGenerateParams(t *testing.T) {
	ctx := t.Context()
	cases := []struct {
		selectFunc                  func(context.Context, *v1alpha1.PullRequestGenerator, *v1alpha1.ApplicationSet) (pullrequest.PullRequestService, error)
	...
	for _, c := range cases {
		...
		generatorConfig := v1alpha1.ApplicationSetGenerator{
...
func TestAllowedSCMProviderPullRequest(t *testing.T) {
	...
	cases := []struct {
		name           string
		providerConfig *v1alpha1.PullRequestGenerator
	}{
		{
			name: "Error Github",
			providerConfig: &v1alpha1.PullRequestGenerator{
