...
func getNestedListGenerator(json string) *appv1alpha1.ApplicationSetNestedGenerator {
	return &appv1alpha1.ApplicationSetNestedGenerator{
		List: &appv1alpha1.ListGenerator{
			Elements: []apiextensionsv1.JSON{{Raw: []byte(json)}},
		},
	}
}

func getTerminalListGeneratorMultiple(jsons []string) appv1alpha1.ApplicationSetTerminalGenerator {
	...
	generator := appv1alpha1.ApplicationSetTerminalGenerator{
		List: &appv1alpha1.ListGenerator{
			Elements: elements,
		},
	}
...
func TestMergeGenerate(t *testing.T) {
	...
	testCases := []struct {
		name           string
		baseGenerators []appv1alpha1.ApplicationSetNestedGenerator
		mergeKeys      []string
		expectedErr    error
		...
		},
		{
			...
			baseGenerators: []appv1alpha1.ApplicationSetNestedGenerator{
				{
					Matrix: toAPIExtensionsJSON(t, &appv1alpha1.NestedMatrixGenerator{
						Generators: []appv1alpha1.ApplicationSetTerminalGenerator{
		...
		},
		{
			...
			baseGenerators: []appv1alpha1.ApplicationSetNestedGenerator{
				{
					Merge: toAPIExtensionsJSON(t, &appv1alpha1.NestedMergeGenerator{
						MergeKeys: []string{"a"},
						Generators: []appv1alpha1.ApplicationSetTerminalGenerator{
	...
	for _, testCase := range testCases {
		...
		t.Run(testCaseCopy.name, func(t *testing.T) {
			...
			got, err := mergeGenerator.GenerateParams(&appv1alpha1.ApplicationSetGenerator{
				Merge: &appv1alpha1.MergeGenerator{
