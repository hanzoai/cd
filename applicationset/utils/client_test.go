...
func TestExecAndSyncCacheNotFoundSkipsNonApplication(t *testing.T) {
	...
	got := c.execAndSyncCache(t.Context(), func() error { return notFound }, &corev1.ConfigMap{Name: "x", Namespace: "cd"}, false)
	require.Error(t, got)
...
func TestExecAndSyncCacheNotFoundHandlesGetStoreError(t *testing.T) {
	...
	app := &application.Application{Name: "test", Namespace: "cd"}
