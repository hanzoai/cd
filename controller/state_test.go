...
func TestCompareAppStateNamespaceMetadata(t *testing.T) {
	...
	assert.Equal(t, map[string]string{"cd.hanzo.ai/sync-options": "ServerSideApply=true", "bar": "bat", "foo": "bar"}, result.GetAnnotations())
	assert.Empty(t, app.Status.Conditions)
}
...
func TestCompareAppStateRequireDeletion(t *testing.T) {
	...
	obj1.SetAnnotations(map[string]string{"cd.hanzo.ai/sync-options": "Delete=confirm"})
	obj2 := NewPod()
	obj2.SetName("my-pod-2")
	obj2.SetAnnotations(map[string]string{"cd.hanzo.ai/sync-options": "Prune=confirm"})
	obj3 := NewPod()
...
func TestCompareAppStateManagedNamespaceMetadataWithLiveNsDoesNotGetPruned(t *testing.T) {
	...
	ns.SetName(test.FakeDestNamespace)
	ns.SetNamespace(test.FakeDestNamespace)
	ns.SetAnnotations(map[string]string{"cd.hanzo.ai/sync-options": "ServerSideApply=true"})
...
func Test_isObjRequiresDeletionConfirmation(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			obj := NewPod()
			obj.SetAnnotations(map[string]string{"cd.hanzo.ai/sync-options": strings.Join(tt.resourceSyncOptions, ",")})
