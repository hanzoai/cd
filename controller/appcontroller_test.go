...
import (
	...
	"github.com/hanzoai/cd/gitops-engine/pkg/cache/mocks"
	...
	mockcommitclient "github.com/hanzoai/cd/commitserver/apiclient/mocks"
	mockstatecache "github.com/hanzoai/cd/controller/cache/mocks"
...
func TestProcessRequestedAppOperation_FailedNoRetries(t *testing.T) {
	...
	assert.Equal(t, "Failed to load application project: error getting app project \"default\": appproject.apps.hanzo.ai \"default\" not found", patchedApp.Status.OperationState.Message)
}
...
func TestProcessRequestedAppOperation_FailedHasRetries(t *testing.T) {
	...
	assert.Contains(t, patchedApp.Status.OperationState.Message, "Failed to load application project: error getting app project \"invalid-project\": appproject.apps.hanzo.ai \"invalid-project\" not found. Retrying attempt #1")
...
// A manual sync that follows an automated one must be judged as manual: under a
// deny window with manualSync it runs, and nothing of the previous operation
// (initiatedBy.automated, retryCount, syncResult) leaks into the new one.
func TestProcessRequestedAppOperation_ManualAfterAutomatedInDenyWindow(t *testing.T) {
	app := newFakeApp()
	app.Spec.Project = "default"
	app.Status.OperationState.Operation.InitiatedBy = v1alpha1.OperationInitiator{Automated: true}
	app.Status.OperationState.Phase = synccommon.OperationFailed
	app.Status.OperationState.Message = "Operation terminated"
	app.Status.OperationState.RetryCount = 2
	require.NotNil(t, app.Status.OperationState.FinishedAt)
	require.NotEmpty(t, app.Status.OperationState.SyncResult.Resources)
	app.Operation = &v1alpha1.Operation{
		Sync:        &v1alpha1.SyncOperation{},
		InitiatedBy: v1alpha1.OperationInitiator{Username: "z"},
	}
	proj := defaultProj.DeepCopy()
	proj.Spec.SyncWindows = v1alpha1.SyncWindows{{
		Kind:         "deny",
		Schedule:     "* * * * *",
		Duration:     "24h",
		Applications: []string{"*"},
	...
	patchedApp, err := ctrl.applicationClientset.AppsV1alpha1().Applications(app.Namespace).Get(t.Context(), app.Name, metav1.GetOptions{})
	require.NoError(t, err)
	state := patchedApp.Status.OperationState
	require.NotNil(t, state)
	assert.False(t, state.Operation.InitiatedBy.Automated)
	assert.Equal(t, "z", state.Operation.InitiatedBy.Username)
	assert.Zero(t, state.RetryCount)
	require.NotNil(t, state.SyncResult)
	assert.Empty(t, state.SyncResult.Resources)
	assert.Equal(t, synccommon.OperationSucceeded, state.Phase)
	assert.Equal(t, "successfully synced (no more tasks)", state.Message)
}
