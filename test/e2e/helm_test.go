...
func TestHelmHooksAreCreated(t *testing.T) {
	...
	ctx.Path("hook").
		...
		Expect(ResourceResultIs(ResourceResult{Version: "v1", Kind: "Pod", Namespace: ctx.DeploymentNamespace(), Name: "hook", Message: "pod/hook created", Images: []string{"ghcr.io/hanzoai/e2e-container:0.1"}, HookType: HookTypePreSync, Status: ResultCodeSynced, HookPhase: OperationSucceeded, SyncPhase: SyncPhasePreSync}))
}
...
func TestHelmHookWeight(t *testing.T) {
	Given(t).
		...
		PatchFile("hook.yaml", `[
	{"op": "replace", "path": "/metadata/annotations", "value": {"apps.hanzo.ai/hook": "Sync", "helm.sh/hook-weight": "-1"}},
