...
func TestAppCreation(t *testing.T) {
	...
	ctx.
		...
		And(func() {
			errors.NewHandler(t).FailOnErr(fixture.AppClientset.AppsV1alpha1().Applications(fixture.TestNamespace()).Patch(t.Context(),
...
func TestAppRollbackSuccessful(t *testing.T) {
	Given(t).
		...
		And(func(app *Application) {
			...
			require.NoError(t, err)
			app, err = fixture.AppClientset.AppsV1alpha1().Applications(fixture.TestNamespace()).Patch(t.Context(), app.Name, types.MergePatchType, patch, metav1.PatchOptions{})
			require.NoError(t, err)
...
func TestCreateAppWithNoNameSpaceForGlobalResource(t *testing.T) {
	Given(t).
		...
		Then().
		And(func(app *Application) {
			app, err := fixture.AppClientset.AppsV1alpha1().Applications(fixture.TestNamespace()).Get(t.Context(), app.Name, metav1.GetOptions{})
			require.NoError(t, err)
...
func TestCreateAppWithNoNameSpaceWhenRequired(t *testing.T) {
	Given(t).
		...
		Then().
		And(func(app *Application) {
			updatedApp, err := fixture.AppClientset.AppsV1alpha1().Applications(fixture.TestNamespace()).Get(t.Context(), app.Name, metav1.GetOptions{})
			require.NoError(t, err)
	...
	Given(t).
		...
		And(func() {
			require.NoError(t, fixture.SetResourceOverrides(map[string]ResourceOverride{
				"apps.hanzo.ai/StatusSubResource": {
					Actions: actions,
				},
				"apps.hanzo.ai/NonStatusSubResource": {
					Actions: actions,
				},
			}))
		}).
...
func TestDeletionConfirmation(t *testing.T) {
	...
	ctx.
		...
		Async(true).
		When().
		PatchFile("guestbook-ui-deployment.yaml", `[{ "op": "add", "path": "/metadata/annotations", "value": { "cd.hanzo.ai/sync-options": "Delete=confirm" }}]`).
		CreateApp().Sync().
		Then().
