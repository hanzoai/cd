...
func testHookSuccessful(t *testing.T, hookType HookType) {
	...
	ctx.
		Path("hook").
		When().
		PatchFile("hook.yaml", fmt.Sprintf(`[{"op": "replace", "path": "/metadata/annotations", "value": {"cd.argoproj.io/hook": %q}}]`, hookType)).
		CreateApp().
		Sync().
		Then().
...
func TestPostSyncHookFailure(t *testing.T) {
	Given(t).
		Path("hook").
		When().
		PatchFile("hook.yaml", `[{"op": "replace", "path": "/metadata/annotations", "value": {"cd.argoproj.io/hook": "PostSync"}}]`).
		// make hook fail
		PatchFile("hook.yaml", `[{"op": "replace", "path": "/spec/containers/0/command/0", "value": "false"}]`).
		CreateApp().
		IgnoreErrors().
...
func TestPostSyncHookPodFailure(t *testing.T) {
	Given(t).
		...
		When().
		IgnoreErrors().
		PatchFile("hook.yaml", `[{"op": "add", "path": "/metadata/annotations", "value": {"cd.argoproj.io/hook": "PostSync"}}]`).
		// make pod fail
...
metadata:
  annotations:
    cd.argoproj.io/hook: Sync
    cd.argoproj.io/hook-delete-policy: HookFailed # To preserve existence before sync
  name: invalid-hook
spec:
  containers:
    - command:
...
metadata:
  annotations:
    cd.argoproj.io/hook: SyncFail
    cd.argoproj.io/hook-delete-policy: HookSucceeded # To preserve existence before sync
...
func TestHookDeletePolicyHookSucceededHookExit0(t *testing.T) {
	Given(t).
		Path("hook").
		When().
		PatchFile("hook.yaml", `[{"op": "add", "path": "/metadata/annotations/cd.argoproj.io~1hook-delete-policy", "value": "HookSucceeded"}]`).
		CreateApp().
		Sync().
		Then().
		Expect(OperationPhaseIs(OperationSucceeded)).
...
func TestHookDeletePolicyHookFailedHookExit0(t *testing.T) {
	Given(t).
		Path("hook").
		When().
		PatchFile("hook.yaml", `[{"op": "add", "path": "/metadata/annotations/cd.argoproj.io~1hook-delete-policy", "value": "HookFailed"}]`).
		CreateApp().
		Sync().
		Then().
		Expect(OperationPhaseIs(OperationSucceeded)).
...
func TestHookBeforeHookCreation(t *testing.T) {
	...
	ctx.
		Path("hook").
		When().
		PatchFile("hook.yaml", `[{"op": "add", "path": "/metadata/annotations/cd.argoproj.io~1hook-delete-policy", "value": "BeforeHookCreation"}]`).
		CreateApp().
		Sync().
		Then().
		Expect(OperationPhaseIs(OperationSucceeded)).
...
func TestHookBeforeHookCreationFailure(t *testing.T) {
	Given(t).
		...
		When().
		PatchFile("hook.yaml", `[
	{"op": "add", "path": "/metadata/annotations/cd.argoproj.io~1hook-delete-policy", "value": "BeforeHookCreation"},
...
func TestHookSkip(t *testing.T) {
	Given(t).
		...
		PatchFile("pod.yaml", `[{"op": "replace", "path": "/metadata/annotations", "value": {"cd.argoproj.io/hook": "Skip"}}]`).
		CreateApp().
		Sync().
		Then().
		Expect(OperationPhaseIs(OperationSucceeded)).
...
func TestAutomaticallyNamingUnnamedHook(t *testing.T) {
	Given(t).
		...
		PatchFile("hook.yaml", `[{"op": "replace", "path": "/metadata/annotations", "value": {"cd.argoproj.io/hook": "PreSync,PostSync"}}]`).
		CreateApp().
		Sync().
		Then().
		Expect(OperationPhaseIs(OperationSucceeded)).
					...
					HealthLua: `
						...
						if obj.metadata.finalizers ~= nil  then
							for i, finalizer in ipairs(obj.metadata.finalizers) do
								if finalizer == "cd.argoproj.io/hook-finalizer" then
...
metadata:
  annotations:
    ...
    cd.argoproj.io/hook-delete-policy: %s
  name: %s
spec:
  containers:
    - command: [ "/bin/sh", "-c", "--" ]
