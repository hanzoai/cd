...
func TestAppSyncWrongVersion(t *testing.T) {
	...
	ctx.
		...
		Expect(SuccessRegex(`The Kubernetes API could not find version "v1alpha2" of hanzoai\.io/Fake for requested resource [a-z0-9-]+/fake-crd-instance\. Version "v1alpha1" of hanzoai\.io/Fake is installed on the destination cluster\.`)).
		When().
		...
		Get().
		Then().
		Expect(SuccessRegex(`The Kubernetes API could not find version "v1alpha1" of wrong\.group/Fake for requested resource [a-z0-9-]+/fake-crd-instance-wronggroup\. Version "v1alpha1" of hanzoai\.io/Fake is installed on the destination cluster\.`)).
		When().
