...
// 'cd.argoproj.io/sync-options: Validate=false' sync option
...
// TestSyncOptionsValidateTrue verifies when 'cd.argoproj.io/sync-options: Validate=false' is
...
func TestSyncWithSkipHook(t *testing.T) {
	...
	Given(t).
		...
		When().
		PatchFile("guestbook-ui-deployment.yaml", `[{ "op": "add", "path": "/metadata/annotations", "value": { "cd.argoproj.io/hook": "Skip" }}]`).
		PatchFile("guestbook-ui-deployment.yaml", `[{ "op": "replace", "path": "/spec/replicas", "value": 1 }]`).
		Refresh(RefreshTypeNormal).
		Then().
		Expect(SyncStatusIs(SyncStatusCodeSynced)).
...
func TestSyncWithForceReplace(t *testing.T) {
	...
	ctx.
		...
		PatchFile("guestbook-ui-deployment.yaml", `[{ "op": "add", "path": "/metadata/annotations", "value": { "cd.argoproj.io/sync-options": "Force=true,Replace=true" }}]`).
