...
func TestClusterSkipReconcileAnnotation(t *testing.T) {
	...
	err := fixture.DoHttpJsonRequest("PUT",
		...
		&cluster,
		fmt.Appendf(nil, `{"annotations":{%q:"true"}}`, "cd.hanzo.ai/skip-reconcile")...)
	require.NoError(t, err)
	assert.Equal(t, "true", cluster.Annotations["cd.hanzo.ai/skip-reconcile"])
	...
	assert.Equal(t, "true", cluster2.Annotations["cd.hanzo.ai/skip-reconcile"])
