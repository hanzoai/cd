...
func TestProgressiveSyncHealthGating(t *testing.T) {
	...
	Given(t).
		...
		And(func() {
			...
			fixture.Patch(t, "progressive-sync/dev/deployment.yaml", `[{"op": "replace", "path": "/spec/template/spec/containers/0/image", "value": "ghcr.io/hanzoai/e2e-container:0.1"}]`)
		...
		And(func() {
			// Patch deployment to use valid image
			fixture.Patch(t, "progressive-sync/staging/deployment.yaml", `[{"op": "replace", "path": "/spec/template/spec/containers/0/image", "value": "ghcr.io/hanzoai/e2e-container:0.1"}]`)
			// Refresh the app to detect git changes
		...
		And(func() {
			// Patch deployment to use valid image
			fixture.Patch(t, "progressive-sync/prod/deployment.yaml", `[{"op": "replace", "path": "/spec/template/spec/containers/0/image", "value": "ghcr.io/hanzoai/e2e-container:0.1"}]`)
			// Refresh the app to detect git changes
