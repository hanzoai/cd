	...
	if err != nil {
		if apierrors.IsNotFound(err) {
			if project != "" {
				...
				return nil, nil, status.Error(codes.NotFound, apierrors.NewNotFound(schema.GroupResource{Group: applicationType.Group, Resource: applicationType.ApplicationPlural}, name).Error())
			}
