...
func (h *Handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if name, ok := r.URL.Query()["name"]; ok && enabled && !notFound {
		...
		if app, err := h.appClientset.AppV1alpha1().Applications(reqNs).Get(r.Context(), name[0], metav1.GetOptions{}); err == nil {
	...
	if projects, ok := r.URL.Query()["project"]; ok && enabled && !notFound {
		...
		if apps, err := h.appClientset.AppV1alpha1().Applications(reqNs).List(r.Context(), metav1.ListOptions{}); err == nil {
